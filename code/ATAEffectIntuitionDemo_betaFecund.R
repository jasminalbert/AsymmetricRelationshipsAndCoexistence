#This script to help explain ATA intuitively. Uses the beta-fecundities lottery 
...
#***
#Calculate E1 and C_{1\1} for the cases of right-tail, left-tail, and symmetric noise, beta-fecundities
#***
...
jpeg(file=paste0(resloc,"PostHocExplanatoryFig_betaFecund.jpg"),width=totwd,height=totht,units="in",quality=90,res=400)
...
#color image showing the function ln(1-delta+E1/C1bs1), for beta-fecundities
#***
...
#label the "tails"
text(-1.3,1,"right",adj=c(0,0),cex=0.65)
text(-1.3,1,"left",adj=c(1,1),cex=0.65)

#label the dashed lines
text(.8,-1.25,expression(L[2]),cex=0.65,adj=c(0,0))
text(-4.5,-4.28,expression(L[1]),cex=0.65,adj=c(0,1))
...
xDEF_rg<-range(log10(bf_E1l/bf_C1bs1l),log10(bf_E1s/bf_C1bs1s),log10(bf_E1r/bf_C1bs1r))
xDEF<-seq(from=xDEF_rg[1],to=xDEF_rg[2],length.out=100)
yDEF<-log(1-bf_delta+10^xDEF)
yDEF_rg<-range(yDEF)
