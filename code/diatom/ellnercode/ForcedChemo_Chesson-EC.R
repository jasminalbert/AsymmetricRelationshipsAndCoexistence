...
#
# Here the partitioning uses Chesson's E and C, E = V(temperature), C =  R/(K+R) 
...
require(deSolve); 
source("ForcedChemoSubs.R"); rm(r1); rm(r2); rm(r1C); rm(r2C); 
# species 1 is Fragillaria, species 2 is Cyclotella 
...
#################################################
E1 = V1fun(minus1$temp);  E1.noVar = V1fun(minus1.noVar$temp); 
C1 = (K1fun(minus1$temp)+minus1$R)/minus1$R 
C1.noVar = (K1fun(minus1.noVar$temp) + minus1.noVar$R)/minus1.noVar$R   

E2 = V2fun(minus1$temp);  E2.noVar = V2fun(minus1.noVar$temp); 
C2 = (K2fun(minus1$temp)+minus1$R)/minus1$R 
C2.noVar = (K2fun(minus1.noVar$temp) + minus1.noVar$R)/minus1.noVar$R   

E1star = mean(E1.noVar); E2star=mean(E2.noVar); C1star=mean(C1.noVar); C2star=mean(C2.noVar); 
E10 = mean(E1); E20 =  mean(E2); C10=mean(C1); C20=mean(C2); 

### "exact sharping" - all possible combinations 
EC1=expand.grid(E1,C1); E1sharp=EC1[,1]; C1sharp=EC1[,2]; rm(EC1); 
EC2=expand.grid(E2,C2); E2sharp=EC2[,1]; C2sharp=EC2[,2]; rm(EC2); 

r1star = rEC(E1star,C1star,parms2); # these are r1(E*,C*), no-variance baseline 
r2star = rEC(E2star,C2star,parms2); 

r10 = rEC(E10,C10,parms);  # r1(E0,C0), mean (E,C) in presence of varying temperature 
r20 = rEC(E20,C20,parms); 

r1sharp = mean(rEC(E1sharp,C1sharp,parms)) # these are rbar(E-sharp,C-sharp)
r2sharp = mean(rEC(E2sharp,C2sharp,parms)) 

r1bar = mean(rEC(E1,C1,parms)); # these are rbar(E,C) 
...
delta1.sigmaE = mean(rEC(E1,C10,parms))-r10; 
delta2.sigmaE = mean(rEC(E2,C20,parms))-r20;

delta1.sigmaC = mean(rEC(E10,C1,parms))-r10; 
delta2.sigmaC = mean(rEC(E20,C2,parms))-r20;
...
#################################################
E1 = V1fun(minus2$temp);  E1.noVar = V1fun(minus2.noVar$temp); 
C1 = (K1fun(minus2$temp)+minus2$R)/minus2$R 
C1.noVar = (K1fun(minus2.noVar$temp) + minus2.noVar$R)/minus2.noVar$R  

E2 = V2fun(minus2$temp);  E2.noVar = V2fun(minus2.noVar$temp); 
C2 = (K2fun(minus2$temp)+minus2$R)/minus2$R 
C2.noVar = (K2fun(minus2.noVar$temp) + minus2.noVar$R)/minus2.noVar$R   
...
## There are no r1 or r2 functions.  Did you mean r1C, r2C?
source("ForcedChemoSubs.R");
