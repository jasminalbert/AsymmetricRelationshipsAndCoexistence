...
fig5 <- function(dat_loc, invader){
  ...
  for (i in 1:3){
    ...
    graphics::rect(resc$var[1], ymin*1.2, tail(resc$var,1)[1], ymax*1.2, col=rescol, border=NA)
    
    excl <- vdat[vdat$ECbrk<vdat$GWR & vdat$GWR<0,]
    graphics::rect(excl$var[1], ymin, tail(excl$var,1)[1], ymax, col=excol, border=NA)
