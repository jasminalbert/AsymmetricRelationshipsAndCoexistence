...
decompose <- function(mu_1,mu_2,sigma_1,sigma_2, delta,blist,dir="LEFT") {
  #notation follows from paper/sup mat
  
  b <- data.frame(blist$l)
  b_til <- data.frame(blist$til)
  b_um <- data.frame(blist$um)
  if (dir=="RIGT"){
  	b <- data.frame(blist$r)
  }
  ...
  #estimate
  eCi_hat <- mean(log(1-delta+delta*(u_i/(sigma_j*b$j + mu_j)))) - e0i
  #standard error
  #eCi_se <- sd()/sqrt(M)
  
  #4. epsilon^(E#C) bar (eq 51)
  #estimate
  e_ECshi_hat <- mean(log(1-delta+delta* ((sigma_i*b_til$i+mu_i)/(sigma_j*b_til$j+mu_j))))- eEi_hat-eCi_hat - e0i
  #standard error
  #eECshi_se <- sd()/sqrt(M)  
  ...
  #estimate
  eCj_hat <- mean(log(1-delta+delta*(u_j/(sigma_j*b$j + mu_j)))) - e_0j
  #standard error
  #eCj_se <- sd()/sqrt(M)
  ...
  eECpsj <- mean(log(1-delta+delta* ((sigma_j*b_til$j+mu_j)/(sigma_j*b_til$i+mu_j))))
  ...
  DECps <- eECpsi_hat - eECpsj              #Deltai_[E||C]
  Dr <- ri_hat 
  DrwoATA <- ri_hat - DECps
  ...
  ei <- c(e0i, eEi_hat, eCi_hat, eECshi_hat, eECi_hat, eECpsi_hat, ri_hat, NA)
  #ei_se <- c(0, eEi_se, eCi_se, eECshi_se, eECi_se, eECpsi_se, ri_se, NA)
  ej <- c(e0j, eEj_hat, eCj_hat, eECshj_hat, eECj_hat, e_ECpsj, rj, NA)
