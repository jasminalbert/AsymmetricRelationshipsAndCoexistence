...
versions differed in the distribution of $(B_1,B_2)$ used. Both distributions were based on 
...
that both versions of the lottery model use (Introduction). The parameter $\sigma$ is the variance of the
log-fecundities and $\mu_i$ is the mean log fecundity for species $i$. 

The \emph{beta-fecundities lottery model}
used $B_i = \eta_i F_\beta \circ \varphi^{-1}(b_i)$, where $\varphi$ is the CDF of the standard normal
...
does not depend on $i$. These choices for $E$ and $C$ meet the assumptions of Theory (SI section Y).
%***DAN: This SI reference is to the big section S$ listed below, but this material will be in the
%first part of that section, prior to the subsections.

\subsection{Plotting results for G2}

%Then write a para or section that starts by saying we plotted the decomp terms against model parameters for
%the log-normal-fecundity lottery model, and some mathematical shortcuts simplified it. Then summarize
%the shortcuts.
%This is similar to an existing para. Also cites a lot of sup mat stuff.

%Then a similar para for the beta-fecundity lottery model.

%Then a para about ATA exclusion and rescue and how that was assess for each model - different from
%the current way of doing it with trait spaces - drop the trait space stuff.
