...
for the lottery model. Mechanisms of coexitence should correspond to
...
%E and vital rates
Whereas the ``$E$'' of ``$EC$-covariance'' is often conceptualized as ``environment'', in fact it is a 
...
nature of ATAs between the population responses which actually matter for coexistence.
Vital rates are often measured directly in demongraphic studies, so ATAs between vital rates could,
in principle, be studied. To our knowledge, this has not yet been done. 
...
The new terms result from rendering one or more of $X$, $Y$, and $Z$ independent of the others.
...
of such a decomposition may present difficulties.
...
%***DAN: DAN: if we take Ellner off the author list add him back here, and also add him back to the acks in the README.
