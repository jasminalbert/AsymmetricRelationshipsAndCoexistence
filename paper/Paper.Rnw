...
the influence of environment-competition correlation \emph{per se}, without 
the influence of ATAs. Simulations of a simple model
...
on average, for the ATA case. Parameters were $\mu_1 = \Sexpr{fig2$params[1]} + \Sexpr{fig2$params[1]}$ and $\mu_2 = \Sexpr{-(fig2$params[1])}$, $\delta = \Sexpr{fig2$params[2]}$, and $\sigma = \Sexpr{fig2$params[3]}$. See Introduction for model details. 
\label{fig:pedagog}}
\end{figure}
