...
instead they depend only on $\eta_1/\eta_2$ (SI section \ref{SI-SIsect:TheoryForLottery}). 
So we plotted against $\eta_1/\eta_2$. 
For each combination of $\delta=0.2, 0.4, 0.6$
and for left- and right-tail associated noise, we plotted 
the components of (\ref{eq:full_decomp}) against the upper bound ratio $\eta_1/\eta_2$. 
The simulation methods of \cite{Ellner_2019} were again used.
Computationally efficient means of performing these simulations are in 
SI section \ref{SI-sect:efficient_computation}.

%***DAN: Next steps as of 2023 02 01:
%DONE Fill in the sup mat section for the theory for the beta fecundities lottery model
%fill in the efficient computation section for that same model
%DONE write this section, here, in the main text
