...
but standard statistical methods for examining the association between two variables provide only 
...
Storage effects allow competing species to fluctuate over time while escaping extinction because storage effects provide a relative benefit to
