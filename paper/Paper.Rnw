...
rare and common species growth rates \citep{Chesson_1994,Ellner_2016}. %invader and resident 
...
two-species model, below, and specifically for the lottery model of the Introduction, 
in SI section \ref{SI-SIsect:TheoryForLottery}.
...
%***DAN: I think we should replace the terminology "invasion growth rate" and "IGR" by "growth when rare" and "GWR",
% throughout the paper. Has to be done in the main text and sup mat and the figures themselves.
% Need also search for "invader" since sometimes we use that word. Also think of any other related terminology that needs to 
% be altered as part of the overall change. 

The storage effect for species $i$ is the contribution of covariation between $E$ and 
$C$ to a difference between rare and common species mean growth rates, when $i$ is the invader.
...
influences of environment and competition on the either species have been rendered independent.  
A subscript 
...
which equals the GWR. The decomposition, which we use below, is similar to and motivated by the 
...
it was then sufficient to decompose the growth when rare of the weaker competitor, species 1. 
...
made it sufficient to decompose only the growth when rare of species 1, for this 
...
The condition $\Delta_i^{[EC]}/\text{GWR}>1$ encompasses the cases for which ATA
...
persistence of \emph{F. crotonensis} and they're sum, the GWR
...
Increasing amplitude and mean temperature showed increasing trends in the GWR and 
...
is always positive, even in cases where GWR is negative, and is almost always second to 
...
Fig.\ref{fig:fig6} displays the subsets of environmental parameters for which coexistence occurs or fails and the degree of ATA contribution to GWR. Specific to this model and parameter ranges, because $\Delta_i^{[EC]}$ is always positive, all positive values of $\Delta_i^{[EC]}/\text{GWR}$ indicate coexistence and negative values of $\Delta_i^{[EC]}/\text{GWR}$ indicate failure of coexistence. Furthermore, ATA exclusion does not occur and ATA rescue occurs in positive cases where $\Delta_i^{[EC]}>\text{GWR}$. To preserve simplicity and readability of the plots, $|\Delta_i^{[EC]}/\text{GWR}>1|$ were capped at $|1|$. Higher magnitudes of $\Delta_i^{[EC]}/\text{GWR}$ tended to occur as GWR became smaller and smaller, thus creating the sharp contrast in the plots where highest intensity positive spaces quickly change into highest intensity negative spaces with decreasing parameters. Like the result of Fig. \ref{fig:fig5}, we see in Fig. \ref{fig:fig6} that amplitude and mean temperature have the most influence on coexistence and the effect of periodicity of temperature fluctuation is small. Coexistence occurs as $\theta_0$ and $a$ increase and $\Delta_i^{[EC]}/\text{GWR}$ changes from negative to positive. 

GWR decomposition of \emph{C. psuedostelligera} across the same subsets of
temperature parameters revealed that its GWR is always positive; though, the ATA contribution is not meaningful to its persistence (see SI section). %Mean EC shows a rescue effect against variance in E
Thus, the coexistence of the two diatom species depends only on the persistence of 
\emph{F. crotonensis} and its ability to increase when rare. Additionally, because ATAs 
meaningfully contribute to GWR of \emph{F. crotonensis}, ATAs then also contribute 
...
coexistence – GWR would be negative without it – deeming the scenario a case of 
...
on average, for the ATA case. Parameters were $\mu_1 = \Sexpr{fig2$params[1] + fig2$params[1]}$,
$\mu_2 = \Sexpr{-(fig2$params[1])}$, $\sigma = \Sexpr{fig2$params[2]}$, and $\delta = \Sexpr{fig2$params[3]}$. 
See Introduction and Fig. \ref{fig:noise} caption for model details. 
\label{fig:pedagog}}
\end{figure}
...
\emph{F. crotonensis} is rare is meaningfully large compared to 
other components of the MCT decomposition. All panels show GWR and the decomposition of
...
coexistence and exclusion of the rare species, \emph{F. crotonensis}. 
...
our defined parameters only depends on GWR of \emph{F. crotonensis} so we only 
show this case; though, an analogous plot (SI Fig. ) was generated for the reverse scenario where \emph{C. pseudostelligera} is rare and 
\emph{F. crotonensis} is common.}  \label{fig:fig5}
\end{figure}

\begin{figure}
\includegraphics[width=.38\textwidth]{../results_figs/fig6.pdf}
\caption{The degree of ATA contribution to GWR change with parameters of environmental variation, i.e., temperature 
oscillation. The ratio $\Delta_i^{[EC]}/\text{GWR}$ constitutes the degree of ATA contribution to GWR with larger 
magnitudes signifying larger ATA contributions. In this model, for the plotted parameters, positive $\Delta_i^{[EC]}/
\text{GWR}$ only result from positive $\Delta_i^{[EC]}$ and positive GWR and negative $\Delta_i^{[EC]}/\text{GWR}$ only 
result from positive $\Delta_i^{[EC]}$ and negative GWR. Thus, cyan areas indicate failure to coexist and magenta areas 
indicate coexistence with ATA contribution increasing with color intensity. The most intense areas of magenta indicate 
parameter subspaces where ATA rescue occurs. For simplicity and readability magnitudes of $\Delta_i^{[EC]}/\text{GWR}$ 
>1 were capped at $|1|$. 
Coexistence in this system and within 
our defined parameters only depends on GWR of \emph{F. crotonensis} so we only 
show this case.} \label{fig:fig6}
\end{figure}
