...
plankton population density time series; many other ecological examples of ATAs have also been explored 
...
MCT breaks down coexistence into the contributions of various mechanisms, with names such as ``storage effects'' \citep{Chesson_1994}; storage effects are reviewed conceptually below and will be
defined formally in the Theory section. MCT has been
...
It is possible to see why it is plausible that
ATAs may influence one mechanism of coexistence in particular, storage effects. We review, conceptually, 
...
Supporting Information (SI). Readers interested in the mathematical aspects should read that version.
%***DAN: Don't forget to do this

%Some intuition
We next give some initial intuition about how ATA effects on coexistence work, 
providing additional intuition in the Discussion.
...
both 1) when $E$ is moderately favorable but $C$ is very strong, as well as 2) when $E$ is 
