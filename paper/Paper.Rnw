...
but standard statistical methods for examining the association between two variables provide only a small 
portion of the overall information content of the
association \citep{Ghosh_2020_AER,Ghosh_2020_Eco,Ghosh_2020_ecolevol,Ghosh_2021}. For instance, 
...
%***DAN: Please search for "growth when rare" and replace with GWR (except for the first instance,
%where GWR is defined). If one defines an acronym, one then has to use it.
...
%***DAN: I see many instances of IGR have been replaced by GWR, but there are some IGRs remaining, e.g., in the sup mat, and
%on the figures.
...
%***DAN: I think ATA rescue and ATA exclusion are mislabeled in the legend of Fig. 4. I think they got switched. Please check and correct if I am right.
...
%***DAN: I ended up deciding we do not need sup mat analogues of Figs. 5 and 6, nor do we need 
%sup mat section S11.

%***DAN: Dan has edited through here and all he figure captions. Still need to do discussion and sup mat
%8-11.
...
Standard errors of plotted quantities were always less then X, so are not shown.
%***DAN: Don't forget to fill in the standard error stuff, please, Jasmin.
...
%***DAN: Jasmin, some changes to this figure are needed. Please make axis labels and numbers bigger. Please put 
%axis labels outside the axes instead of inside. Please make panel labels bigger. Please make coutour labels
%slightly bigger.
\begin{figure}
...
excluding ATAs, so GWR for \emph{F. crotonensis} is a sufficient coexistence metric.} \label{fig:fig6}
\end{figure}
