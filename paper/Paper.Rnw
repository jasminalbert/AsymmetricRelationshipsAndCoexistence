...
Notation for the whole paper is summarized in Table X.
%***DAN: Dan to make the table
...
ATAs substantially influenced aspects of species coexistence in these simulations (Fig. \ref{fig:pedagog1}d-f).
...
G3) To provide an empirical example of species coexistence which highlights the role ATAs can play and the
...
\subsection{Removing asymmetries of association}
...
and ``ATA exclusion''. \emph{ATA rescue} was considered to have occurred for a given scenario if coexistence
was possible when ATAs were taken into account, i.e., GWR$>0$ for both species, but impossible otherwise, i.e.,
the sum of the non-ATA terms in (\ref{eq:full_decomp}) was negative for at least one species. \emph{ATA exclusion} was 
considered to have occurred if the species did not coexist when ATAs
were taken into account, i.e., GWR$<0$ for at least one species, but would have coexisted were it not for ATAs, 
i.e., the sum of the non-ATA terms in (\ref{eq:full_decomp})
was positive for both species.
...
To pursue goal G2 of the Introduction, we applied the framework of Theory to the lottery model, 
...
of the lottery model which we now specify. The versions both used the same dynamical equations
(Introduction) and assumed temporally independent and identically distributed (iid) fecundities, but the
...
with shape and scale parameters $0.5$ (probability density function proportional to $x^{-05}(1-x)^{-0.5}$ on
the unit interval). So the beta fecundities model had parameters $\eta_1$, $\eta_2$ and $\delta$,
and produced fecundities with $U$-shaped, beta distributions between $0$ and $\eta_1$, for $B_1$, and
...
%***DAN: Jasmin mentioned that she only used \delta=0.2, .4, .6 because of a comment Ellner made where he 
%said it was known that larger values of \delta made for very little storage effects. Read that comment 
%again and then write something here to jstify the choice. I think I can probably then delete the section of
%sup mat, mentioned in the commented text below, that talks about \delta=1.
...
the fairly realistic range $1 \leq \eta_2/\eta_1 \leq 5$. 
...
%***DAN: This para will likely have to be revised depending on how Jasmin revises the diatom figure presentation
To display results for goal G3, we plotted the terms of (\ref{eq:full_decomp}) against one of the parameters 
...
tended to be negative, weakening total storage effects. The ATA term 
was also often comparable in magnitude to the other terms of 
...
for realistic parameter values for the beta fecundities lottery model. For instance, the ATA in 
Fig. \ref{fig:fig3} was fairly 
small, compared to the other terms of (\ref{eq:full_decomp}), for values of $\sigma$ less than 
about $3$. And ATA rescue and exclusion also only occurred for $\sigma$ greater than about $3$.
...
for the log-normal fecundities lottery model using realistic parameter values. However, 
...
to the other components of (\ref{eq:full_decomp}) across the whole range of $\eta_2/\eta_1$ values
considered; and both ATA rescue and exclusion occurred for values of this ratio between $1$ and $2$.
Apparently the question of whether ATAs are important for realistic parameter values depends on model specifics.

%G4 ***DAN: Dan, this will have to be edited after Jasmin gets done editing the corresponding figures
...
The authors especially thank Steve Ellner for key advice at several watershed moments in the devlopment of the project. 
...
%***DAN: This looks pretty good except the histograms for some marginals do not really look like the plotted
...
\caption{Illustration of how asymmetric tail associations are removed while retaining marginal distributions and correlation 
...
%***DAN: Jasmin, this figure looks really great. Here are a few minor things I would like to ask you to
%do to further optimize:
%1) Currently there are axis labels and numbers only on the bottom and left panels, which makes sense.
%But the axis tick marks needs to occur on all panels. I noticed that this would be an imporvement
%when I found myself wanting to see where the ATA line was between the x axis and the GWR line, but it was 
%a little hard to see where the x axis was becuase there were no tick marks. It is possible to have tick marks
%but no numbers of labels in R, and that's what we need here.
%2) The font of the x axis label, "log fcundity standard deviation, \sigma" seems slightly different from the 
%other textual labels. Can you please make it consistent? Perhaps it is not bold and the others are bold, or 
%something.
\begin{figure}
...
are plotted against $\sigma$. The contribution of ATAs ($\Delta_i^{[EC]}$) frequently 
...
%***DAN: Jasmin, panel labels(a, b, c, etc.) need to be added. Please use the same format for these labels
%as you do in the other figures.
\begin{figure}
...
are plotted against $\eta_2/\eta_1$. The contribution of ATAs ($\Delta_i^{[EC]}$) frequently 
has magnitude comparable to other components, indicating that ATAs can contribute meaningfully
to species coexistence or failure to coexist. In some instances, the GWR line falls 
...
coexistence and exclusion of the weaker competitor. These instances, which are shaded, correspond to
ATA rescue or exclusion (Theory). Standard errors of plotted quantities 
were never greater than \Sexpr{round(NA,5)}, so are not displayed. 
%***DAN: Jasmin, we need to replace the NA in the above line by a variable names, which your code should have saved.
%The variable needs to be loaded in the R chunk near the top of this file, and then refereced here, please.
This plot uses $q_{ij}=1$. 
%***DAN: Right, Jasmin?
An analogous plot (SI Fig. \ref{SI-fig:fig4_alt}) was generated 
using $q_{ij}=\eta_i/(\eta_j(1-\delta)+\eta_i \delta)$. 
%***DAN: Jasmin, I believe the code does not produce the alternate-qij plot yet, but eventually it needs to.
Though results differed in detail, main conclusions were the same.
%***DAN: make sure this is indeed the case once the plot is generated.
}\label{fig:fig4}
\end{figure}
