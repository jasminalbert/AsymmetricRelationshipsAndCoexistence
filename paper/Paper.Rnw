...
%ATA specialists
The contributions we have documented of ATAs to species coexistence open the possibility that
``ATA specialists'' may exist in some contexts. If a stronger competitor is present across a
landscape and the GWR of a weaker competitor %***DAN: Dan is working right here.
...
among many from the appropriate term with parenthesized superscripts with the $\shp$ symbol denoting and separating 
...
For example, the term $\Delta^{(XY \shp Z)}$ could be decomposed into $\Delta^{[XY \shp Z]} + \Delta^{[X \nat Y \shp Z]}$. 
...
%***DAN: DAN: if we add Ellner to the author list take him out here, and also take him out of the acks in the README.
