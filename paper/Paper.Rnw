...
to a competitive diatom model show that ATA contributions to storage effects
...
Futhermore, though storage effects are typically described as the contribution
of covariance between drivers to coexistence, commonly referred to as \emph{EC covariance},
...
Not only is the contribution of ATA non-neglible but in some cases the presence of absence of ATA 
...
driving mechanism of the failure of coexistence and the lack of ATAs would result in 
...
Distinct increases in the magnitude of ATA contribution with increasing
environmental fluctuations, as shown in our results and
figures (Fig. \ref{fig:fig3} - \ref{fig:fig6}), suggests that ATAs and their effects
...
patterns of environmental variables in nature can
...
the contributions of correlation per se. In newer, simulation based MCT, storage effect is 
the sum of the two new terms, $\Delta^{(EC)}=\Delta^{[E \nat C]}+\Delta^{[EC]}$, obscuring the effects of ATAs.
