...
symmetric type of association, while Fig. \ref{fig:pedagog1}a,c exhibit what has previously been referred to as 
...
MCT breaks down coexistence into the contributions of various mechanisms, with names such as ``storage effects'' \citep{Chesson_1994} which will be familiar to some readers (storage effects are also defined below). MCT has been
