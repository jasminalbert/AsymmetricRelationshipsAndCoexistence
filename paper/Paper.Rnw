...
population growth rate. To study such relationships and to understand how two variables interact, ecologists 
...
regression), but correlations are extremely common, and other common methods also reveal only a small portion 
...
$C_i(t)$. Though the iid assumption is not necessary, it simplifies the
...
used by \cite{Descamps_2005}. We again labelled regions of parameters space corresponding to
...
Apparently the question of whether ATAs are important for realistic parameter values depends on specifics.

\subsection{Goal G3, results for the diatom system}

%***DAN: check this again after Jasmin gets done making the new version of Fig. 5
...
ATA rescue occurs for some combinations of parameters for this system (Fig. \ref{fig:fig5}a-c).

%***DAN: finish it once Jasmin finishes d-f
Plotting against two parameters at a time, Fig. \ref{fig:fig5}d-f show that...
...
quantifies the impact of asymmetric tail associations (ATAs) on species coexistence and the maintenance of biodiversity. 
...
based example (the diatom system). Though future work should seek to understand precisely when ATAs are or are not important
for coexistence, our work demonstrates the overall importance of this new mechanism.
...
ATAs in the same way (\ref{eq:full_decomp}) extends (\ref{eq:previous_decomp}). See SI section \ref{SI-sect:threeway} for details.
...
%***DAN: Jasmin, please add to the list anyone you talked to at UO about this, and anyone who read it and commented 
%for you. Be generous here - even short convos merit an acknowlegement.
The authors thank Shyamolina Ghosh for suggesting the research problem, and Lawrence Sheppard, Max Castorani,
Jon Walter, Jude Kastens, Vadim Karatayev, Emily Arsenault, Neil Thimmesch, Laurn Hallett, and 
Robin Snyder for helpful discussions. 
...
%Also please add tick marks to all axes (see comment on previous figure)
\begin{figure}
...
%***DAN: Jasmin, can you please ensure that ATA exclusion and rescue are 
%shaded and keyed in the legend for panels a-c in the same way as the earlier figures?
\begin{figure}
\includegraphics[width=\textwidth]{../results_figs/fig5.pdf}
\caption{ATA effects and ATA rescue and exclusion in the diatom system. 
...
%***DAN: Change the above sentence if Jasmin changes the font of the line
ATA contributions ($\Delta_i^{[EC]}$)
had magnitude comparable to other components, 
indicating that ATAs can contribute meaningfully
to diatom coexistence in this system. In some instances, the GWR line falls 
...
%***DAN: Once Jasmin is done with the other part of the figure (the part that used to be Fig. 6), 
%finish this caption. Use the below caption, currently commented, to help. 
}  \label{fig:fig5}
\end{figure}
