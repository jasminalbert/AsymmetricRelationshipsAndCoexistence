...
such as early- and late-spring rainfall if, as seems likely, meteological events which
are more extreme also tend to be longer lasting. The possibility 
...
and how they differ across space and may be changing through time due to human
impacts on the climate. Extreme environmental events have been much studied, including
how the nature of such events may be changing with climate change, and how such changes 
may influence species coexistence (see citations in the Introduction). But
studies of changes in environmental ATAs would differ from such prior work because
ATAs pertain to relationships between variables in the extremes, a potential changes in those
relationships, to the best of our knowledge, have not yet been examined.  

%E and vital rates
Whereas $E$ of the $EC$ associatons which produce storage effects 
is often conceptualized as ``environment'', in fact it is a 
...
of the lottery model is actually better conceptualized as a species response
than as the environment, directly.
Likewise, one of the choices of $E_i$ considered by \cite{Ellner_2016} for the
diatom model was $E_i=V_i$, the maximum population growth rate, clearly a response
to the environment rather than a direct measurement of the environment. 
...
Thus the nature of ATAs between directly measured environmental variables may differ from the
nature of ATAs between the population quantities which actually matter for species coexistence.
Vital rates are often measured directly in demongraphic studies. To our knowledge, the 
nature of tail associations between vital rates has been studied seldom or never. 
...
Storage effects as computed using the original, analytic appoach to MCT 
...
original, analytic appproach, which are well known and also elaborated by \cite{Ellner_2019};
one such strength is the generality that analtic approaches provide.
...
&+ \Delta_i^{(XYZ)}+\Delta_i^{(XY \shp Z)}+\Delta_i^{(X \shp YZ)}+\Delta_i^{(XZ \shp Y)}+\Delta_i^{(X \shp Y \shp Z)}.
\end{align}
The terms in (\ref{eq:9}) and (\ref{eq:10}) are analogous to those of (\ref{eq:previous_decomp}).
The new terms result from rendering one or more of $X$, $Y$, $Z$ independent of the others.
...
of such a decomposition may present some difficulties.
...
ecological and environmental variables \citep{Ghosh_2020_AER}. 


%***DAN: stopped here 2022 04 21
