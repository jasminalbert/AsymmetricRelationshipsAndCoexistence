...
The consequence of climate change that has the most potential to influence ATAs in the environment the increase in intensity, duration, extent, and frequency of extreme climatic events, i.e., heat-waves, floods, hurricanes, fires, etc. 
...
Environmental fluctuations are directly impacted by climate change through changes in means and increased variance \citep{Hansen_2012}, influenced by extreme climatic events as they increase in frequency, intensity, duration, and extent \citep{Meehl_2004, Ummenhofer_2017}. 
...
Currently, effects on competition and coexistence due to climate change are more widely researched through studying changes in the mean of environmental variables, thus less likely to be a result of extreme events. 
...
To further demonstrate plausibility of ATA altering coexistence between species, we carried out simulations using a simple two-species model, the lottery model.  We introduce the lottery model and describe simulations based on it that clearly show that ATAs can influence 
...
lottery model, and below (in the next paragraph), we describe a hypothetical example. Environmental variability has been classically shown to promote coexistence via storage mechanisms in a temporal lottery model of competition \citep{Chesson_1981, Warner_1985}, hence we move forward with this simple model to examine the effect of extreme relationships between environmental responses on the contribution of storage effects to coexistence. We simulated two different communites up to \Sexpr{M} time steps to hopefully make it clear to the reader that ATA makes a difference for competition and coexistence between species (see Figure \ref{fig:pedagog}). The only distinction between the two simulations is that one depends on asymmterically tail associated temporal dynamics and one does not. Markedly, the simulation with ATA has longer periods of one-species dominance than the simulation without, implying less recoverablility when rare and thus less coexistence \citep{Chesson_2000} when environmentally dependent variables are asymmetrically tail associated.     
...
a simple example model, the lottery model. 
