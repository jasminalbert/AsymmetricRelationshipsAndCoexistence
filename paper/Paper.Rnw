...
total abundance, are marked in grey. The average duration of periods of noticiable coexistence 
...
on average, for the ATA case. Parameters were $\mu_1-\mu_2 = \Sexpr{fig2$params[1]}$, 
%***DAN: Jasmin, at this point of the ms, the reader does not know that only the difference mu1-mu2 matters, so this
%will confuse them. Please replace the above specfication of mu1-mu2 by a specification of which values of both
%mu1 and mu2 were actually used for this simulation. 
$\delta = \Sexpr{fig2$params[2]}$, and $\sigma = \Sexpr{fig2$params[3]}$. See Introduction for model details. 
\label{fig:pedagog}}
\end{figure}
