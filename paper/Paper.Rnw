...
\begin{abstract}
\noindent Understanding the coexistence of competing species is a topic of very broad 
interest in ecology. The modern coexistence
theory (MCT) of Chesson identifies several mechanisms of species coexistence,
one of which, storage effects, hinges on relationships between fluctuations
in environmental and competitive pressures on species populations. However, such relationships
have typically been measured using covariance methods, which do not account for 
the possibility that environmental and competitive influences may be more correlated
when these influences are strong than when they are weak, or vice versa. 
Recent work showed that such `asymmetric tail associations' (ATAs) are extremely common
between ecological variables, and are important for a wide range of phenomena including 
extinction risk and the stability of ecosystem functioning. 
Here we extend the MCT to take into account the influence of ATAs. Storage
effects can be decomposed into two components, one of which quantifies the
influence of ATAs on species coexistence, and is a novel contribution of this study; and 
the other of which represents
the influence of environment-competition correlation \emph{per se}, 
without the influence of ATAs. Simulations of a simple model
%and an empirical example
illustrate that ATA influences on coexistence can be comparable in magnitude to other
mechanisms of coexistence, and that the presence or absence of ATAs can make the 
difference between species coexisting and competitive exclusion. 
\end{abstract}

%***Add to these
\keywords{asymmetric tail association, biodiversity, coexistence, competition, modern coexistence theory, storage effects}
...
It is well known that climate change can or may alter the intensity, duration, extent and frequency of extreme environmental events, including
heat waves, floods, hurricanes and fires \citep{Jentsch_2007, Meehl_2004, Ummenhofer_2017}.  
...
not been studied, the importance for coexistence of other aspects of climate change, including changes 
in means and measures of variability of environmental variables, has been studied in many systems, so we briefly 
review some of that large, related literature.
For instance, \cite{diPaola_2012} concluded that coexistence of drought sensitive,
...
by projected changes in mean rainfall for the region. In an interesting example
involving climate-change effects on herbivore-mediated competition in plants,
...
herbivores at high altitudes can inflate species richness by reducing dominant plant species 
...
and, as a result, some species would be extirpated. Numerous studies also take an ``event-focused'' 
...
\cite{White_2001} showed that single extreme heating events can alter competition 
between C3 and C4 plants. And \cite{Parepa_2013} showed that extreme variability in soil
nutrient availability for plants, which is mediated by soil moisture content, can alter 
...
%Goal para
We previously demonstrated \citep{Ghosh_2020_AER} that ATAs exist and are common in nature, and we argued above 
...
from ATAs compared to other, known mechanisms of coexistence. This is to help illuminate whether ATA contributions to coexistence 
are likely to be negligible or meaningfully large. 
G3) To explore, using our simple model, whether species can coexist with
...
To achieve goal G4 from the Introduction, i.e., to provide an empirical example of species coexistence which highlights the role ATAs can play, we consider the coexistence of diatom species established by chemostat batch experiments \citep{Descamps_2005}. \citep{Descamps_2005} used measurements of growth of \emph{Cyclotella pseudostelligera} and \emph{Fragilaria crotonensis} over a range of temperatures to parametrize a competitive resource model as follows:
put model here
Via simulations, \cite{Descamps_2005} showed that coexistence of \emph{C. pseudostelligera} and \emph{F. crotonensis} occurs with oscillating temperature fluctuations, $\theta(t)=\theta_0 (t) +  a\sin(2\pi t/P)$, defined by parameters $\theta_0 = 18, a = 6, P = 60$d. \cite{Ellner_2016, Ellner_2019} broke down their coexistence into the contrirbuting mechanisms, and here, we consider varying parameters of temperature fluctuations and further decompose the coexistence of \emph{C. pseudostelligera} and \emph{F. crotonensis} to account for contributions of ATAs and ATA effects as described above. Our decomposition is defined by $E(t)=\theta(t)$, $C_j(t)=(K_j(t)+S(t))/S(t)$ and $r_j(E_j,C_j)=\frac{V_j(E)}{C_j} -D$. Thus in our exploration, environmental fluctuation is temperature fluctuation. We first considered the scenario where \emph{F. crotonensis} is the invader and emph{C. pseudostelligera} is the resident to compute the coexistence decomposition for \emph{F. crotonensis}. 
%put reverse in sup mat?
%do we need to explain how we separated the asymmetries using the sims??
Like for G2, we first plotted the terms of equation \ref{eq:full_decomp} for the chemostat model against independent parameters defining temperature oscillation, $\theta_0, a$, and $P$, a total of three plots. For each plot, we set two of the parameters constant at the original parameters which coexistence was experimentally found in \cite{Descamps_2005}, and let the remaining temperature parameter vary. 
...
To further explore the parameter space, we generated two dimensional surface plots which reveal the subsets of parameter space where coexistence is possible with ranging degrees of ATA contributions and where ATA rescue occurs. We evaluated coexistence under two varying parameters and indicated $\Delta_i^{[EC]}/\text{IGR}$, a ratio of ATA contribution to total IGR, with a color bar and contour lines. Greater magnitudes of $\Delta_i^{[EC]}/IGR$ and higher color intensity indicate greater contribution of ATAs to the IGR and positive cases where $\Delta_i^{[EC]}>IGR$ indicate ATA rescue. 

All computations were done in R on an ordinary laptop. Complete computer codes for the project are stored at
...
%The second one is the one to cite
...
alternative patterns of association in the tails. A) is ``left-tail 
associated,'' B) is symmetric, and C) is ``right-tail associated'' noise. All three cases have 
the same Pearson correlation, $\rho$, up to sampling variation, yet show very different 
patterns of association. We used asymmetrically tail associated cases which are perfectly 
correlated in the lower (A)  or upper (C) halves of the distributions, though it is also 
...
\end{figure}
%***DAN: Dan has edited this caption at least once.
...
on average, for the ATA case. Parameters were $\mu_1 = \Sexpr{fig2$params[1] + fig2$params[1]}$ and $\mu_2 = \Sexpr{-(fig2$params[1])}$, $\sigma = \Sexpr{fig2$params[2]}$, and $\delta = \Sexpr{fig2$params[3]}$. See Introduction for model details. 
\label{fig:pedagog}}
\end{figure}
%***DAN: Dan has edited this caption
...
\caption{The contribution of ATAs to diatom coexistence is meaningfully large compared to 
other components of the MCT decomposition. All panels show the IGR and the decomposition of
the IGR provided by the extended MCT, equation (\ref{eq:full_decomp}). For each panel, 
two of the three parameters defining temperature oscillation, amplitude, period, and mean, 
is fixed at the original coexistence values found by \cite{Descamps_2005} ($a=6, P=60, \theta_0=18$). The IGR and components of the decomposition
are plotted against one varying parameter. The original value of the varying parameter and coexistence quantities at that value is indicated by the vertical dashed magenta lines. The contribution of ATAs (red line)
...
between the ATA line (red)
and the $x$ axis, indicating that, for those parameter values, ATAs make the difference between species
coexistence and exclusion of the weaker competitor. Standard errors of plotted quantities ?.}  \label{fig:fig5}
\end{figure}
