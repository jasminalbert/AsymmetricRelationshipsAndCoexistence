...
$C$ to a difference between invader and resident species mean growth rates, when $i$ is the invader.
...
\end{equation}
which is the extension of MCT which was goal G1 of the Introduction. One typically %(but see below) 
must examine this quantity for each species, $i$,
