...
two-species model, below, and specifically for the lottery model of the Introduction, in SI section \ref{SI-SIsect:TheoryForLottery}.
...
It is assumed that the growth rate $r_i(t)=\ln(N_i(t+1)/N_i(t))$ 
...
%the figure panels themselves). Need to also search for "invader" since sometimes we use that word. Also think of any 
%other related terminology that needs to be altered as part of the overall change.
...
A subscript $i \bs i$ always indicates a quantity computed for species $i$ when that species is at negligible abundance, and 
...
\cite{Ellner_2016} and SI section \ref{SI-SIsect:scaling_factors} for the two alternative definitions of $q_{ij}$
that we use. 
...
For those familiar with copula statistics (see, e.g., \citealp{Ghosh_2020_AER} and \citealp{Genest_2007}), 
