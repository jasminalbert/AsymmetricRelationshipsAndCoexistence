...
values on the plots. Some mathematical shortcuts, summarized here, greatly simplified this plotting. First, we showed 
that, for the 
...
as well as the
IGR minus its ATA contribution, $\olr_{i \bs i} -\Delta_i^{[EC]}$. 
...
coexistence which highlights the role ATAs can play, we use a laboratory system of freshwater diatoms
explored by \cite{Descamps_2005}.
\citep{Descamps_2005} used measurements of growth of \emph{Cyclotella pseudostelligera} 
and \emph{Fragilaria crotonensis} over a range of temperatures to parametrize a competitive 
resource model as follows.
Letting $\frac{dS}{dt}$ and $\frac{dx_j}{dt}$ denote the rates of resource uptake and reproduction for $j=1,2$ the
model equations are
\begin{equation}
\frac{dS}{dt} = D(S_0 - S) - Q_1x_1 \frac{V_1S}{K_1 + S} - Q_2x_2 \frac{V_2S}{K_2 + S}
 \frac{dx_j}{dt} = x_j \frac{V_jS}{K_j+S}-D_xj, $j=1,2$.
 \end{equation}
 Here, $S$ is a extracellular silicate concentration, $S_0$ is the inflow concentration, $D$ is the outflow rate, and $x_j$ 
 are the diatom densities. The temperature-dependent variables for resource consumption, $Q_j$, reproduction rate, 
 $V_j$, and half-saturation constant, $K_j$ were obtained by fitting lines and curves to values obtained from the chemostat batch experiments \cite{Descamps_2005, Ellner_2016}. 
%***DAN: Jasmin, please add the model. Hard to evaluate the text without it.
%***JASMIN: Done
After using lab experiments to parameterize the model, \cite{Descamps_2005} use simulations to
show that coexistence of \emph{C. pseudostelligera} 
and \emph{F. crotonensis} occurs when temperature fluctuates periodically, 
$\theta(t)=\theta_0 +  a\sin(2\pi t/P)$, 
%***DAN: Jasmin, should \theta_0 depend on t? It should not, right? Please fix if I'm right.
%***JASMIN: You're right, it should not. Corrected
with parameters $\theta_0 = 18^\circ$C, $a = 6^\circ$C, $P = 60$d. 
%***DAN: I added units. Please check.
%***JASMIN: Checked. Done. 
\cite{Ellner_2016, Ellner_2019} broke down their coexistence into the contributing mechanisms, 
...
As for G2, we first plotted the terms of equation \ref{eq:full_decomp} for the chemostat 
model against independent parameters defining temperature oscillation, $\theta_0, a$, and 
$P$, a total of three plots. For each plot, we set two of the parameters constant at the 
original parameters which coexistence was experimentally found in \cite{Descamps_2005}, 
and let the remaining temperature parameter vary. 
We let $\theta_0$ vary in $[16,19]$, $a$ vary in $[1,6]$, and $P$ vary in $[20,120]$. 
Relative importance of contributions was judged from the 
plots to assess whether and to what extent ATA effects on coexistence were meaningfully 
large for the lottery model, compared to 
the contributions of other mechanisms. 
The simulation methods of \cite{Ellner_2019} were used to compute the components of (\ref{eq:full_decomp}).

To further explore model parameter space, we generated two dimensional surface plots which reveal 
some subsets of parameter space where coexistence is possible with ranging degrees of ATA 
contributions and where ATA rescue occurs. We evaluated coexistence under two varying 
parameters and indicated $\Delta_i^{[EC]}/\text{IGR}$, a ratio of ATA contribution to total 
IGR, with a color bar and contour lines. Greater magnitudes of $\Delta_i^{[EC]}/\text{IGR}$ and 
higher color intensity indicate greater contribution of ATAs to the IGR and positive cases 
where $\Delta_i^{[EC]}>\text{IGR}$ indicate ATA rescue. 
...
can coexist with ATAs, but not without, or vice versa), our results show that ATA can, indeed, sometimes
...
more common for larger $\delta$. Although bands of ATA rescue or exclusion on Fig.
...
show that the ATA contributions have magnitude comparable to the 
...
All panels show the IGR, the ATA term of the extended MCT [$\Delta_i^{[E || C]}$ in equation 
...
\emph{F. crotonensis} is the resident.
}  \label{fig:fig5}
\end{figure}
