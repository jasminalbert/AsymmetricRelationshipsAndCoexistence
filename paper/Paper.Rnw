...
%***DAN: Jasmin, Descamps_2005 does not appear in refs.bib, I think. Please add it there or subtract the callout here, whatever is most appropriate.
...
%***DAN: Methods paras here for G4, if we accomplish G4
To achieve goal G4 from the Introduction, i.e., to provide an empirical example of species 
coexistence which highlights the role ATAs can play, we consider the coexistence of diatom 
species established by chemostat batch experiments \citep{Descamps_2005}.
...
Via simulations, \cite{Descamps_2005} showed that coexistence of \emph{C. pseudostelligera} 
and \emph{F. crotonensis} occurs with oscillating temperature fluctuations, 
$\theta(t)=\theta_0 (t) +  a\sin(2\pi t/P)$, 
%***DAN: Jasmin, should \theta_0 depend on t? It should not, right? Please fix if I'm right.
defined by parameters $\theta_0 = 18, a = 6, P = 60$d. 
\cite{Ellner_2016, Ellner_2019} broke down their coexistence into the contrirbuting mechanisms, 
...
%***DAN: Yes, I think that would be a good idea. I forgot about that - we were able to just consider 1 invading 2 for 
%the lottery model, but cannot really do that here, right?
...
%***DAN: Jasmine, some changes are needed to the figure itself. Please make the text in the legend bigger.
...
and mean, $\theta_0$) are fixed at the original coexistence values used by 
\cite{Descamps_2005} ($a=6^\circ$C, $P=60$d, \theta_0=18^\circ$C). 
%***DAN: I added units for a  and \theta_0 here, please check these are correct 
The IGR and components of the decomposition
are then plotted against the other parameter. The original value of the varying parameter and coexistence quantities at that value is indicated by the vertical dashed magenta lines. The contribution of ATAs (red line)
has magnitude comparable to other components and, in these parameters, is always positive, indicating that ATAs can contribute meaningfully
to diatom coexistence. In some instances, the IGR line (orange) falls 
between the ATA line (red)
and the $x$ axis, indicating that, for those parameter values, ATAs make the difference between species
coexistence and exclusion of the weaker competitor. Standard errors of plotted quantities ?.
