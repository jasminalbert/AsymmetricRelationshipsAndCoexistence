...
Storage effects allow competing species to fluctuate %over time while escaping extinction because storage effects provide 
in abundance without any going extinct by providing
a relative benefit to whichever species is currently rare; this is the characteristic that makes storage effects a so-called \emph{stabilizing} mechanism 
...
We now specify how to decompose storage effects into contributions due to ATAs and 
...
Therefore, storage effects are the only mechanism of coexistence
