...
%Some intuition
We next give the beginnings of some intuition about how ATA effects work, with a fuller understanding
necessarily coming only later in the paper.
It is known that storage effects can occur if some buffering mechanism reduces losses in bad years so that
...
$C$ are aligned with the differences in the potential for storage across the dichotomy 1 versus 2.
...
We spend most of the paper formalizing ATA effects in MCT and demonstrating to what 
extent ATA effects can be strong versus weak, but we return in the Discussion to the task
of building an intuition for how ATAs work. 
