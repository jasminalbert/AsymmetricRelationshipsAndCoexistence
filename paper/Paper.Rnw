...
$\text{IGR}-\Delta_i^{[E || C]}$ is negative correspond to ATA rescue (shaded yellow), whereas
values of $\mu_1-\mu_2$ for which the IGR is negative but $\text{IGR}-\Delta_i^{[E || C]}$ is positive 
