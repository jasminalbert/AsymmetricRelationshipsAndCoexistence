...
indicate the IGR when the effects of ATAs are included, and blue lines are the IGR when
the effects of ATAs are neglected. These two lines frequently crossed the $x$ axis in 
different places, showing that both ATA rescue space and ATA exclusion space could be non-empty
...
\caption{The contribution of ATAs to species coexistence is meaningfully large compared to 
...
were never greater than \Sexpr{round(se3,5)}, so are not displayed. \label{fig:fig3}}
\end{figure}
...
using $q_{ij}=\exp(\mu_i)/[(1-\delta)\exp(\mu_j)+\delta\exp(\mu_i)]$, but it looked exactly the same, by mathematical necessity (see SI section ).}\label{fig:fig4}
\end{figure}
%***DAN: Dan has edited this caption
