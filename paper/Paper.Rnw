...
distributed (iid) through time. In three distinct simulations (Fig. \ref{fig:pedagog1}d-f), 
...
G1) We will augment MCT to show formally how ATAs play a role in coexistence. 
...
G2) We will explore, using the simple lottery model, the relative magnitudes 
...
G3) We will provide an empirical example of species coexistence which highlights the role ATAs can play and the
...
%***DAN: Dan made it to here on 2022 04 26
...
%***DAN: Jasmin, could you, with reasonable effort, move panels around? Specifically, put the panels which
%are now g ang h in the middle instead of on the right (and they will then be called panels d and e) and 
%put what are now panels d-f on the right (and thwill then be called f-h)? The reason is, we refer to
%g-h before we refer to d-f, and the reader may become confused by this, including possibly havig trouble 
%finding the parts of the caption that refer to g-h while getting confused about the parts of the caption 
%that refer to d-e, which are about something that has not even been introduced at the point at which the 
%reader first is referred th panels g-h. 
\begin{figure}
...
%2) The font of the x axis label, "log fecundity standard deviation, \sigma" seems slightly different from the 
...
\caption{ATA effects and ATA rescue in the diatom system. 
