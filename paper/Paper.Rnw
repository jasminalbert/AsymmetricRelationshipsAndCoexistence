...
put model here
\cite{Descamps_2005} used simulations to show that coexistence occurs with oscillating temperature fluctuations with parameters $\theta_0 = 18, a = 6, P = 60$d) and \cite{Ellner_2016, Ellner_2019} broke down their coexistence into the contrirbuting mechanisms. Here, we consider varying parameters of temperature fluctuations and further decompose the coexistence of \emph{C. pseudostelligera} and \emph{F. crotonensis} to account for contributions of ATAs and ATA effects as described above, using decomposition defined by $E=...$ and $C=...$. 

Like for G2, we first plotted the terms of equation \ref{eq:full_decomp} for the chemostat model with $E=$ and $C=$ against independent parameters of temperature oscillation, $\theta_0, a$, and $P$, a total of three plots. For each plot, we set two of the parameters constant at the original parameters which coexistence was found in \cite{Descamps_2005}, and let the remaining temperature parameter vary. %say which scenario this is in Ellner 2018
