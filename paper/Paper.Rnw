...
invader (rare) and resident (stable-state) species growth rates \citep{Chesson_1994,Ellner_2016}. %invader and resident 
...
is not neccesary, %we make the assumption because 
...
for both invader and resideent species [see (\ref{eq:storage_effects_gen})]. 
...
Specifically, asymmetries of tail association are removed -- see below for how this is done. 
...
of storage effect, which are themselves a term in the MCT decomposition \citep{Ellner_2019} -- see 
