...
\noindent \textbf{Words in Abstract:} 150; \textbf{Words in main text:} 4998; \textbf{Words in text boxes:} 0\\
\noindent \textbf{Number of references:} 40; \textbf{Number of figures:} 5; \textbf{Number of tables:} 1; \textbf{Number of text boxes:} 0\\
...
\noindent \textbf{Data accessibility statement:} All codes associated with this study have been released in a 
public repository.
...
Ecologists ask, for instance, how local environment relates to 
population growth rate. To study relationships, ecologists 
commonly use metrics such as correlation. But standard measures of the strength 
and direction of association between variables do not tell the whole story 
\citep{Ghosh_2020_AER}.
To illustrate, the three pairs of variables in Fig. \ref{fig:pedagog1}a-c have the same 
Pearson correlation despite striking differences in association structure: Fig. \ref{fig:pedagog1}b shows a 
symmetric type of association, while Fig. \ref{fig:pedagog1}a,c exhibit what has been called 
``asymmetric tail association'' [ATA; \citealp{Ghosh_2020_AER}]. 
Metrics of association other than correlations are also used (e.g., various forms of 
regression), but other common methods also reveal only a small portion 
of the information in the relationship \citep{Nelsen2006}.
ATAs and related ideas had previously been seldom considered in ecology, but recently, 
insights were gained by studying how features of associations beyond standard measures 
impact ecological phenomena \citep{deValpine_2014, Anderson_2019, Popovic_2019, Ghosh_2020_AER, Ghosh_2020_Eco,
Ghosh_2020_ecolevol, Ghosh_2021}. \cite{Ghosh_2020_Eco} argued that extinction risks can become elevated 
...
than are years of good environment, limiting rescue effects. 
And \cite{Ghosh_2021} showed that ATAs alter the stability of 
ecosystem functioning. 
Fig. \ref{fig:pedagog1}d,e show contrasting examples of ATAs using 
plankton population density time series;
these examples are intended only to empirically illustrate what ATAs look like, as they
use data of a different type from the focus of this paper.
Many other examples of ATAs in nature have also been explored 
\citep{Ghosh_2020_AER,Ghosh_2020_ecolevol,Ghosh_2021}. To our knowledge, the implications of ATAs for 
competition have not been investigated. Doing so is our goal here. 

%Intro to competition, coexistence, MCT, etc.
Competition and coexistence are topics at the core of modern ecology 
...
drive the other extinct \citep{Hardin_1960}.
Modern coexistence theory [MCT; \cite{Chesson_1994,Chesson_2000,Barabas_2018}] is a 
framework for understanding how species nevertheless coexist in real ecosystems,
through the niche partitioning effects of environmental variation.
%moved the following two sentences from below
MCT has been
...
Although MCT is mathematically complex, recent extensions make it more accessible \citep{Ellner_2016,Ellner_2019}.
%
MCT breaks down coexistence into the contributions of various mechanisms, with names such as 
``storage effects'' \citep{Chesson_1994}. 
Storage effects, which are reviewed conceptually below and defined formally in Theory, refer to
a type of niche partitioning which allows species to effectively ``store" population growth 
benefits from favorable periods in order to persist through unfavorable periods. 

%Intro to storage effects and why they might be influenced by ATAs
We indicate why it is plausible that
ATAs may influence one mechanism of coexistence in particular, storage effects, starting by conceptually reviewing 
what storage effects are \citep{Chesson_1994,Chesson_2012,Ellner_2016,Barabas_2018}. 
Storage effects allow competing species to fluctuate in abundance, without any going extinct,
by providing a relative benefit to whichever species is currently rare \citep{Chesson_2000}.
Storage effects hinge on covariation between the fluctuating quality of the environment
for a species ($E$) and the competition ($C$) experienced by that species (called $EC$ covariance);
and on the assumption that no two species respond identically to environmental variation. Then, good environmental
...
the species limits competitive influence. The resulting asymmetry is what provides a benefit to the
rare species. Storage effects can be quantified by measuring how much $EC$ covariance contributes to 
differences between growth rates of currently rare versus currently common species \citep{Chesson_1994,Ellner_2016}.  
ATAs may, in principle, influence storage effects simply because storage effects hinge on $EC$ covariation, 
and ATAs can be an aspect of $EC$ covariation. One scenario in which it is reasonable to imagine that
ATAs may be present in $EC$ covariation pertains to water limitation. Suppose two plants compete primarily for 
water, when water is scarce, but their niches become differentiated when water is abundant.
Then the quality of the environment, $E$, as experienced by whichever plant is currently rare, may only correspond closely to the 
competition, $C$, imposed on it by the currently abundant plant when water is scarce and $E$ is low.   

%Lottery model and results based on it
To further demonstrate the plausibility of ATAs altering coexistence, we carried out 
simulations using a two-species model, the lottery model. The simulations described in this
paragraph demonstrate that ATAs influence coexistence in some manner; the
nature of that influence is the subject of the rest of the paper.
The lottery model, which was introduced as a model of coral reef fish competing for a set of $N$ territories, 
is as follows.
...
in proportion to the reproductive outputs of the species that year. Note that $N = N_1(t)+N_2(t)$ 
is constant. 
We assume that the random variables $(B_1(t),B_2(t))$ are independent and identically 
...
tail associated, while the same marginal distributions were used in all 
simulations (see Methods
for details). Thus, the setup was identical in the three simulations except for different ATAs. 
ATAs substantially influenced aspects of coexistence in these simulations (Fig. \ref{fig:pedagog1}f-h).
Notation for the paper is summarized in Table \ref{tab:notation}.
Typesetting of mathematics by ecological journals is sub-optimal,
so, for mathematical readers, we appended a latex version of this main text to the end of the Supporting Information (SI).
%***DAN: Don't forget to do this

%Some intuition
We next give some intuition about how ATA effects on coexistence work, 
with details in the Discussion.
It is known that storage effects can occur if a buffering mechanism reduces population 
losses in bad years so that
gains in good years can more than make up for them. 
Bad years happen when the negative effects of $C$ outweigh any 
positive effects of $E$; but this occurs
both 1) when $E$ is moderately favorable but $C$ is very strong, and 2) when $E$ is 
unfavorable and $C$ is not too weak.
Conversely, good years happen when positive effects of $E$ outweigh negative effects of $C$;
but this occurs 1) when $E$ is favorable and $C$ is not too strong,
and 2) when $E$ is moderately unfavorable but $C$ is very weak. 
It turns out that the potential for storage events to occur when 
$E$ and $C$ are both higher than average (the cases 1, above); versus the potential  
when $E$ and $C$ are
both lower than average (cases 2) are not necessarily equal -- storage potential may differ
across the spectrum from 1 to 2.
ATA effects quantify the extent to which asymmetries of association between $E$ and 
$C$ are aligned with differences in the potential for storage across the spectrum from 1 to 2.
If $E$ and $C$ are largely unassociated in the parts of their distribution where
storage potential is high, ATA effects will be high. ATA effects quantify how temporal niche partitioning
varies across the $EC$ spectrum spanned by the cases 1-2 above.
...
indicate that ATAs may influence coexistence. Thus, we here seek to quantify the consequences 
of ATAs for competition and coexistence, via the following specific goals. 
...
G2) We explore, using the lottery model, the relative magnitudes 
of the contributions to coexistence that come
from ATAs compared to other, previously known mechanisms of coexistence; and we determine whether species may 
...
are negligible or meaningfully large. 
G3) We provide an empirical example of species coexistence which highlights the role ATAs can play and their
potential importance.
The example is a laboratory diatom system. In the Discussion, we consider whether climate change may alter
ATAs of environmental variables and thereby influence competition.
Overall, our study presents a new mechanism of species coexistence
and a means of understanding its importance.

\section{Theory}\label{section:theory}

We here pursue goal G1: to augment MCT to show the role of ATAs for coexistence. 

\subsection{Setup}

We first develop our theory for a general two-species model.
Our notation parallels
\cite{Ellner_2016,Ellner_2019}. It is assumed that the growth rate $r_i(t)=\ln(N_i(t+1)/N_i(t))$ 
of species $i$ can be written as an increasing function of
an environmental factor $E_i(t)$ and as a decreasing function of 
...
$C_i(t)$. Though this assumption is probably unnecessary, it simplifies the
presentation of new ideas. 

\subsection{The storage effect}

MCT and its computational extensions \citep{Ellner_2016,Ellner_2019} 
quantify the contributions of 
multiple mechanisms to coexistence. But we show below
that only one mechanism relates to ATAs: storage effects. 
...
\end{equation}
where: $\olr_{i \bs i}$ is the mean ``growth when rare'' (GWR) of species $i$, that is, the mean growth rate of 
species $i$ when it is at negligible abundance and species $j$ (the other species) is at stochastic steady state;
$\olr_{j \bs i}$ is the mean steady-state 
growth rate of species $j$ when species $i$ is absent or at negligible abundance (equals $0$, \emph{a priori});
...
influences of environment and competition have been rendered independent.  
A subscript 
$i \bs i$ indicates a quantity computed for species $i$ when that species is at negligible abundance, and 
$j \bs i$ indicates a quantity computed for $j$ when $i$ is absent.
...
alternative definitions of $q_{ij}$ we use; but one definition is $q_{ij}=1$, and to understand the 
...
modified model for which $E$ and $C$
were rendered independent [see (\ref{eq:storage_effects_gen})]. 
Because components were 
completely independent in the modified model, storage effects quantify the contribution of the 
whole $EC$ relationship to the rare/common species growth rate
...
and with some but not all aspects of the $EC$ relationship 
altered. Specifically, ATAs are removed -- see below for how. We then define 
...
\end{align}
The term $\Delta_i^{[EC]}$ is the \emph{ATA effect on coexistence}. 
...
Note that brackets, $[\cdot]$, and parentheses, $(\cdot)$, are used distinctly
throughout the paper - they are not interchangeable.
The notation $\nat$ was chosen because this new symbol is part of the symbol $\shp$ (two of its four lines), 
corresponding to the removal of part of the relationship between $E$ and $C$ components (ATAs).
The symbol should be pronounced ``partial sharp''.
See SI section \ref{SI-SIsect:techcomm} for technical comments on the notation. 
...
of storage effects, which are a term in the MCT decomposition \citep{Ellner_2019} -- see 
below for details.
...
$C_{i \bs i}$, which we denote $F_{E_i}$ and $F_{C_{i \bs i}}$, are strictly monotonic and continuous. 
...
Let $\rho_i$ be their covariance. We then define a bivariate normal random variable $(e_i,c_{i \bs i})$ such that
...
The value of $\ddot{\rho}_i$ is determined in one of 
four ways: either by setting $\ddot{\rho}_i = \rho_i$; or so that the Pearson, Spearman or Kendall correlation of 
...
$E_i$ and $C_{i \bs i}$, according to whichever definition of correlation is preferable. Thus $E_i^\nat$ and 
...
defined above (following \cite{Ellner_2016}).

The question of uniqueness of $(E_i^\nat,C_{i \bs i}^\nat)$ 
is addressed fully in SI section \ref{SI-SIsect:uniqueness}.
Briefly, a choice 
is made as to what type of correlation (e.g., Pearson, Spearman, etc.) is to be preserved. 
Additionally, another choice involves the use of a bivariate normal distribution $(e_i,c_{i \bs i})$; 
any other symmetrically associated bivariate 
distribution with standard normal marginals could be used instead.
But the normal distribution is a natural choice, and other choices may 
be somewhat esoteric (SI section \ref{SI-SIsect:uniqueness}). 
Future work could explore the consequences of this choice.

\subsection{Our decomposition as an extension of modern coexistence theory}

The other mechanisms in MCT make no reference to covariation between quantities, whereas ATAs are 
about covariation. Therefore, storage effects are the only mechanism of coexistence
for which ATAs are relevant. The \cite{Ellner_2019} version of MCT comprises 
a decomposition of the coexistence metric $\olr_{i \bs i} -q_{ij}\olr_{j \bs i}$,
which equals GWR of species $i$, as 
\begin{equation}
...
$\Delta_i^{(E\shp C)}$ is a contribution of having simultaneous variation 
in both environment and competition, but not including the effects 
of covariation in these quantities; and
$\Delta_i^{(EC)}$ is a contribution of $EC$ covariation itself -- storage effects. 
Combining our decomposition 
(\ref{eq:storage_effects_further}) with (\ref{eq:previous_decomp}) of \cite{Ellner_2019} gives 
\begin{equation}
...
\end{equation}
which is the extension of MCT that was goal G1. 
The derivation of (\ref{eq:full_decomp}) is in SI section 
\ref{SI-SIsect:other_mechanisms}. 

\subsection{ATA rescue and exclusion}

To evaluate the importance of ATAs for a given scenario,
we quantified
the magnitude of the new ATA effect term, $\Delta_i^{[EC]}$, relative to
the other terms (see Methods). But we also defined scenarios of ``ATA rescue''
and ``ATA exclusion''. \emph{ATA rescue} is considered to have occurred if coexistence
is possible when ATAs are taken into account, i.e., GWR$>0$ for both species, but impossible otherwise, i.e.,
the sum of the non-ATA terms in (\ref{eq:full_decomp}) is negative for at least one species. \emph{ATA exclusion} occurs 
if the species do not coexist when ATAs
...
For goal G2 of the Introduction, we applied our theory to the lottery model, 
comparing the magnitude of the ATA effect on coexistence, $\Delta_i^{[EC]}$, to the magnitudes 
of the other terms of 
(\ref{eq:full_decomp}) for various model parameterizations. We used two versions 
of the lottery model. The versions both used the same dynamical equations
(Introduction), but 
differed in the distributions of $(B_1,B_2)$ used. Both distributions were based on 
transformations of the distributions $(b_1,b_2)$ of Fig. \ref{fig:pedagog1}a-c;
...
(Introduction). The parameter $\sigma$ is the standard deviation of the
...
with shape and scale parameters $0.5$. So the beta fecundities model had parameters $\eta_1$, $\eta_2$ and $\delta$,
and produced fecundities with $U$-shaped distributions between $0$ and $\eta_1$, for $B_1$, and
between $0$ and $\eta_2$, for $B_2$. The mean fecundity for species $i$ was $\eta_i/2$, and larger $\eta_i$ also 
corresponded to more variable fecundities. 

%more stuff that applies to both
For both versions of the lottery model, 
fecundities directly reflected 
...
$C_i$ was taken to be the number of new offspring
divided by the number of open sites, $C_i(t)=( B_1(t)N_1(t)+B_2(t)N_2(t) )/(\delta N$), which
does not depend on $i$. These choices fulfill the assumption of Theory 
that $r_i(t)$ is an increasing function of $E_i(t)$ and a decreasing function of $C_i(t)$ 
...
to coexistence of ATAs versus other mechanisms, we plotted the terms of the decomposition 
(\ref{eq:full_decomp}) against model parameters $\mu_1$, $\mu_2$, $\sigma$ and $\delta$.
...
Second, we showed that none of the components of (\ref{eq:full_decomp}) depends independently on $\mu_1$ and $\mu_2$;
they depend only on $\mu_1-\mu_2$ (SI section \ref{SI-SIsect:TheoryForLottery}). 
...
So we only generated plots for left-tail associated noise. 

For each combination of $\delta=0.2, 0.4, 0.6$ and $\mu_1-\mu_2=0,-0.5,-2,-4$,
we plotted the components of (\ref{eq:full_decomp}) against $\sigma$ for $\sigma$ 
from $0$ to $7$. \cite{Chesson_1981} showed that coexistence is impossible for $\delta=1$, and it
is known that the storage effect is weak for larger $\delta$, so we 
only considered $\delta$ up to $0.6$. Some of 
the $\mu$ differences and $\sigma$ values we considered were unrealistic (see Results),
so we considered after plotting whether conclusions depended 
on realistic parameter values.
Simulation methods of \cite{Ellner_2019} were used to compute the components of (\ref{eq:full_decomp}).
Computationally efficient means of performing simulations are in 
...
to coexistence of ATAs versus other mechanisms, we plotted the terms of the decomposition 
(\ref{eq:full_decomp}) against model parameters $\eta_1$, $\eta_2$, and $\delta$,
again labelling regions of ATA rescue or exclusion.
Mathematical shortcuts, summarized here, simplified plotting. 
First, we assumed, without loss of generality, that $\eta_1 \leq \eta_2$. 
It was then sufficient to decompose GWR of the weaker competitor, species 1. 
Second, we showed that none of the components of (\ref{eq:full_decomp}) depends independently on $\eta_1$ and $\eta_2$;
...
Simulation methods of \cite{Ellner_2019} were again used, with 
computationally efficient means of simulating in 
SI section \ref{SI-sect:efficient_computation}.
...
coexistence which highlights the role ATAs can play, we used a laboratory 
...
\emph{Cyclotella pseudostelligera} over a range of temperatures to parameterize a resource
...
$i=2$, $j=1$. Details of the model setup are in SI section \ref{SI-sect:diatom_model}
and specifics of how our decomposition was computed are in 
SI section \ref{SI-sect:Empircal_app}.

\subsection{How results were plotted for goal G3}

%One-parameters plots
To display results for G3, we started by plotting the terms of 
(\ref{eq:full_decomp}) against one of the parameters 
$\theta_0$, $a$ and $P$ at a time, keeping the other two fixed at the values
...
For those plots, the quantity  
$\Delta_i^{[EC]}/|\text{GWR}|$ was displayed using color. Larger magnitudes
of this quantity indicate greater importance of ATAs. The case
$\Delta_i^{[EC]}>\text{GWR}>0$ (which produces $\Delta_i^{[EC]}/|\text{GWR}|>1$)
corresponds to ATA rescue, and $\Delta_i^{[EC]}<\text{GWR}<0$
(which produces $\Delta_i^{[EC]}/|\text{GWR}| < -1$) corresponds to ATA exclusion.
Thus $\Delta_i^{[EC]}/|\text{GWR}|$ indicates the importance of ATAs
and also whether ATA rescue or exclusion occurred.

All computations were done in R on a laptop running Ubuntu Linux. 
The only data used were the phytoplankton time series of Fig. 1, taken from
\url{https://doi.org/10.5061/dryad.rq3jc84}.
Complete computer codes and data are stored publicly as indicated in the Data Availability Statement.
...
of the contributions of ATAs compared to other coexistence mechanisms; and to see if ATA rescue 
or exclusion occur), Fig.
...
magnitude comparable to the magnitudes of other terms. 
...
tended to be negative, weakening storage effects. 

For the beta fecundities lottery model, the ATA term 
was often or always, for the parameters we considered, comparable in magnitude to the other terms of 
the decomposition (\ref{eq:full_decomp}) (Fig. \ref{fig:fig4}). The term contributed negatively to 
coexistence for environmental noise/fecundities
showing left-tailed association, and positively for right-tailed 
association. 

Our results also show that ATAs can make the difference between coexistence 
and competitive exclusion: both ``ATA rescue'' and ``ATA
...
line and the $x$ axis for some values of $\sigma$, e.g., panel i.
For such cases, the presence or absence of ATAs determine whether the species coexist or not. 
For panel i, for values of $\sigma$ around $6$, 
species 1 is excluded with ATAs but the two species coexist without ATAs. 
Fig. \ref{fig:fig4} highlights additional examples using the beta fecundities lottery model,
where examples were more common than for the log-normal fecundities model.  

Contrasting the two models, ATAs were only important for the log-normal fecundities lottery model 
when that model was parameterized unrealistically; but ATAs were important for a wide range of 
realistic parameter values for the beta fecundities model.
For instance, the ATA term in Fig. \ref{fig:fig3} had large magnitude compared to the other
terms of (\ref{eq:full_decomp}), and ATA rescue and exclusion occurred, only for 
$\sigma$ greater than about $3$. 
...
However, all the $\eta_2/\eta_1$ ratio values we plotted for the beta fecundities model 
were fairly realistic, corresponding to
upper-bound fecundities that were up to $5$ times higher for species 2 than for species 1.  
The ATA term in Fig. \ref{fig:fig4} was comparable in magnitude 
to other components of (\ref{eq:full_decomp}) across the whole range of parameters
considered. And ATA rescue and exclusion occurred for values of $\eta_2/\eta_1$ between $1$ and $2$.
...
show that ATA contributions to coexistence often have magnitudes comparable to those 
of other terms of the extended MCT decomposition, and that 
ATA rescue occurs for some parameters for this system.

Plotting against two parameters at a time, Fig. \ref{fig:fig5}d-f reinforce the 
same conclusions. The quantity $\Delta_i^{[EC]}/|\text{GWR}|$ was often large,
indicating that ATAs were a substantial contributor to coexistence. 
For instance, well over half the area of Fig. \ref{fig:fig5}f showed values of 
...
We developed and applied a new extension of modern coexistence theory (MCT) that 
...
can be comparable in magnitude to contributions of previously recognized mechanisms.
Our results add
coexistence to the list of ecological phenomena on which 
...
our results suggest that storage effects should be redefined as the 
contribution of \emph{associations} between $E$ and $C$, generally, including ATA effects and
...
in some contexts, but were very important in other contexts, including our empirically
based example (the diatom system).
Our work therefore demonstrates the potential for ATAs to be an important mechanism of coexistence, generally.
Future work should seek to understand precisely when ATAs are or are not important
for coexistence. 

%Para continuing to try to build intuition about how ATAs work
We return to the task, begun in the Introduction, of building intuition for how
ATA effects work. 
Just as for storage effects [\cite{Chesson_1994}, Box 2 of \cite{StumpVasseur2023}], 
temporal niche differentiation is necessary for ATA
effects (SI section \ref{SI-SIsect:temporal_niche_diff}). 
If temporal niche differentiation occurs
but is partial, as will be typical, $E_i$ and $C_{i \bs i}$ for a 
species $i$ will be positively but imperfectly associated, 
and the association may be asymmetric. 
Storage events occur for species $i$ when the positive effects of $E_i$ overwhelm the negative 
effects of $C_{i \bs i}$, producing large values of $r_i(E_i,C_{i \bs i})$
which are not completely offset by population declines at other times. 
For instance, for the lottery model,
$r_i(E_i,C_{i \bs i})=\log(1-\delta+E_i/C_{i \bs i})$ can get large whenever
$C_{i \bs i}$ is a small fraction of $E_i$, but can never get smaller
than $\log(1-\delta)$. 
The potential for storage events can differ when $E_i$ and $C_{i \bs i}$ are both
above their median values compared to when they are both below median. For instance,
the ratio $E_i/C_{i \bs i}$, and therefore the growth rate 
$\log(1-\delta+E_i/C_{i \bs i})$, can both get very big, corresponding to
rapid growth, if $C_{i \bs i}$ is close to $0$, even if $E_i$ is fairly small.
In contrast, if $E_i$ and $C_{i \bs i}$ are both in the right portions of their distributions, 
so that $C_{i \bs i}$ is bounded away from $0$, there may be much sharper limits on the 
ratio $E_i/C_{i \bs i}$ and the growth rate.
ATA effects quantify the extent to which asymmetries of association between $E$ and 
$C$ are aligned with varying potential for storage across the $E,C$ distribution. 
ATA effects help quantify different degrees of temporal niche differentiation 
occurring under different sets of environmental conditions, possibly via
different mechanisms.
Further details and examples of this intuition 
are in SI section \ref{SI-SIsect:intuition_examples} and SI 
Figs \ref{SI-fig:beta_fecundidities_explanatory}-\ref{SI-fig:lognormal_fecundidities_explanatory}, 
where comparisons are also made between our two lottery models. 

%Climate change may alter ATAs of coexistence-relevant ATAs
An additional motivation for quantifying the importance of ATAs for coexistence 
...
whereas late spring temperature influences species B. If these species are competitors, then $EC$ covariation
influencing their potential coexistence could boil down to covariation
...
early spring temperature and $E_j$ representing late spring temperature, see SI section \ref{SI-SIsect:ATA_specialists}). 
...
longer lasting increases the right-tail association between early and late spring temperature.
As we showed in this study, changes in ATAs can then
result in different competitive outcomes.
...
Additional discussion points are in SI sections \ref{SI-SIsect:ATA_specialists}-\ref{SI-sect:storage_importance}.

%close
It seems reasonable to suppose that ATAs may play an increasingly important role in systems of 
greater complexity because such systems have more pairs of temporally variable quantities that
may exhibit ATAs. Our prior work shows that ATAs are common in 
ecological and environmental variables \citep{Ghosh_2020_AER}. We have here used mathematical
...
such as predation and facilitation, and other complexities.

\section{Author contributions}

Both authors contributed about equally to all parts of the paper.
...
All code and data for the project are available at \url{https://zenodo.org/record/8386993}
%DAN: update when you know
and \url{https://github.com/jasminalbert/AsymmetricRelationshipsAndCoexistence}.
...
in the North Sea considered by \cite{Ghosh_2020_ecolevol} and \cite{Sheppard2019}. ATAs were statistically
...
(f-h) Lottery model population simulations % using $(B_1,B_2)$ 
obtained by transforming the respectively corresponding variables (a-c; see Introduction).
\label{fig:pedagog1}}
\end{figure}
...
See Table 1 for used annotations and abbreviations.
\label{fig:pedagog2}}
\end{figure}
...
in detail, main conclusions were the same. 
See Table 1 for used annotations and abbreviations. \label{fig:fig3}}
\end{figure}
...
See Table 1 for used annotations and abbreviations. }\label{fig:fig4}
\end{figure}
...
\noindent Figure 5 caption. ATA effects and ATA rescue in the diatom system. 
...
sufficient coexistence metric. See Table 1 for used annotations and abbreviations.
