...
The \emph{log-normal fecundities lottery model} used $B_i = \exp(\sigma+\mu_i b_i)$,
and therefore had parameters $\sigma$, $\mu_1$ and $\mu_2$ in addition to the death rate parameter $\delta$
(Introduction) that both versions of the lottery model used. The parameter $\sigma$ is the variance of the
...
\subsection{How results were plotted for G2}
...
%***DAN: Not sure if this will the right citation anymore after we restructure those parts of the SI
So we plotted against $\mu_1 - \mu_2$. 
Third, we showed that the values of all components in (\ref{eq:full_decomp}) were the same 
for both our left- and right-tail associated noises (SI section \ref{SI-sect:math_simplifications}). 
%***DAN: Also check this citation
So we only generated plots 
for left-tail associated noise. For each combination of $\delta=0.2, 0.4, 0.6$
and $\mu_1-\mu_2=0,-0.5,-2,-4$,
we plotted the components of (\ref{eq:full_decomp}) against $\sigma$. 
...
%***DAN: check the citation after restructuring the SI
...
%For the sup mat, I think you will need these sections:
%S3 - on generating the Fig. 1 noise (only)
%S4 - as large section with subsections. About the theory for the two lottery models, but not about the effecient computation
%   Starts with material on the general lottery model
%   Then a sub-section with all the computations for the log-normal-fecundities lottery model
%   Then a sub-section for the beta-fecundities lottery model
%S5 - then a similar section, but for the efficient computations
%There might be additional sections, see the current sections and see if those need to be maintained
%as their own sections or can be absorbed into the above.
%This is going to take some work. 

\subsection{Diatom chemostat system}

%Then on to the diatom stuff.
...
%***DAN: I edited the caption (2023 01 26) but this is the completely the wrong figure. I need to replace it 
%with Jasmin's new pedagogical figure once I know where in the repo 
%structure that one is generated and stored, and after I have re-run Jasmin's code, and after she has made the
%changes I recently asked for (see next comment).
%***DAN: I asked Jasmin to replace the \phi with \varphi in the axis labels on b and d, and to make the marginals
%on b and c be *standard* normal instead of just normal.
\begin{figure}
