...
\affil[b]{Department of Ecology and Evolutionary Biology, Cornell University}
...
one of which, the storage effect, hinges on relationships between fluctuations
...
We extend MCT, decomposing the storage effect to show the influence of ATAs.
...
\keywords{asymmetric tail association, biodiversity, coexistence, competition, competitive exclusion, modern coexistence theory, niche differentiation, storage effect}

\section{Introduction}\label{section:introduction}

%Intro to ATAs
Every ecological discipline involves the study of relationships among biological and environmental variables,
but standard %statistical methods 
measures for %examining the 
measuring the strength and direction of the association between two variables provide only 
%a small portion of the overall information content of the
partial information about the nature of the 
association \citep{Ghosh_2020_AER,Ghosh_2020_Eco,Ghosh_2020_ecolevol,Ghosh_2021}. 
For instance, consider the three pairs of variables in Fig. \ref{fig:noise}.
The variables in Fig. \ref{fig:noise}a,c exhibit what we have previously referred to as ``asymmetric tail association'' 
[ATA; \cite{Ghosh_2020_AER}], i.e., they are more strongly related to each
other in the left (Fig. \ref{fig:noise}a, termed ``left-tail association'') or right (Fig. \ref{fig:noise}c,
termed ``right-tail association'') portions of the variables' distributions. 
The relationships displayed in Fig. \ref{fig:noise} demonstrate very different types of association, 
but nevertheless all have the same Pearson correlation. 
Other metrics of association are also often used, in addition to correlations 
...
Although ATAs and related ideas had previously been seldom 
...
MCT breaks down coexistence into the contributions of various mechanisms, %with names such as ``relative nonlinearityin competition'' and ``storage effect'' \citep{Chesson_1994} which will be familiar to many readers. 
highlighting the importance of environmental fluctuations to species coexistence. 
For example, one such mechanism, the storage effect, promotes coexistence when species can reap or "store" the benefits of times when environmental conditions are good to overcome competition and persist through bad conditions.
MCT has been applied to several systems, including annual \citep{Angert_2009, Hallett_2019} and perennial plants \citep{Adler_2006, Adler_2010}, 
...
Though MCT's original analytical form presented mathematical challenges for applications to and tests of new systems, recent developments expand its testability and accessibility and present the opportunity for further extensions \citep{Ellner_2016,Ellner_2019}. 


%Intro to storage effects and why they might be influenced by ATAs
In this paper, we show how ATAs can also be important for competition and for the coexistence of
competing species. In particular, we show how ATAs can strongly influence one specific coexistence mechanism:
storage effects.
%It is straightforward to see, intuitively, why ATAs may influence one mechanism of coexistence in particular: the storage effect. 
%We review, conceptually, what the storage effect is \citep{Chesson_1994,Chesson_2012,Ellner_2016,Barabas_2018}, and explain why ATAs may be relevant to them.
Storage effect allows competing species to fluctuate %over time while escaping extinction because storage effects provide 
in abundance without any going extinct by providing
a relative benefit to whichever species is currently rare; this is the characteristic that makes the storage effect a so-called \emph{stabilizing} mechanism 
of coexistence \citep{Chesson_2000}.
Storage effect can occur if three conditions are met \citep{Chesson_1994}. First, there must be positive 
covariation between the fluctuating quality of the environment (E)
for a species and the strength of competition (C) experienced by that species (called EC covariance), that is stronger when the species is at stable-state abundance than when it is rare. 
Second, growth responses to the environment must be species-specific, so that distinct species do not find the 
same environmental conditions to be optimal. 
Third, a buffering mechanism %must exist whereby species limit 
which limits the influence of competition %in times 
when the environment is poor. 
Dormant stages such as seed banks are an example buffering mechanism \citep{Chesson_2012}. 
...
Critically, the storage effect can be quantified in invasion analysis by measuring to what extent EC covariance contributes to differences between 
invader (rare) and resident (steady state) species growth rates \citep{Chesson_1994,Ellner_2016}. %invader and resident 
Thus ATAs may, in principle, influence storage effect simply because storage effect hinges on EC covariation, and ATAs, when they 
...
%To further demonstrate the plausibility of ATAs altering coexistence between species, 
To more robustly demonstrate the role of ATAs in structuring fluctuation-dependent coexistence, we carried out simulations using a simple two-species model, the lottery model. 
The simulations described in this paragraph are sufficient to demonstrate that ATAs influence coexistence in some manner; 
the precise nature of that influence is then the subject of the rest of the paper.
The lottery model was originally introduced as a model for coral reef fish competing for
a set of $N$ breeding territories.
...
The model postulates that adults die at rate $\delta$ at each time step, and are replaced by new juveniles 
...
We here pursue goal G1 from the Introduction: to augment MCT to illuminate how ATAs
%plays a role in coexistence. 
can structure coexistence. We develop each step of our new theory for a general 
two-species model %(below)
, and also specifically for the lottery model of the Introduction 
...
multiple mechanisms to coexistence. But we show
that, for purely temporal variation, only one mechanism relates to tail associations: the storage effect. 
So we begin by defining the storage effect. Our mathematical notation parallels
...
$C_i(t)$. Though the iid assumption %can be circumvented
is not necessary, %we make the assumption because 
it simplifies the presentation of new ideas. 

In MCT, coexistence is assessed by the ability to increase when rare, i.e., 
the mean ``growth when rare'' [GWR] of species $i$, 
quantified by the weighted difference between mean growth rates when species $i$, the invader, is at negligible abundance 
and species $j$, the resident (the other species) is at stochastic steady state. The GWR is computed for each species assessed as the invader of a resident community, and only when the difference is positive for all is coexistence possible. 
The weighted difference, $\olr_{i \bs i} -q_{ij}\olr_{j \bs i}$, can be decomposed into coexistence mechanisms. 
A subscript $i \bs i$ indicates a quantity computed for species $i$ when that species is at negligible abundance, 
and a subscript $j \bs i$ indicates a quantity computed for $j$ when $i$ is absent. 
The \emph{scaling factor} $q_{ij}$ weights the relative sensitivities to competition of the two species, 
see \cite{Chesson_1994}, \cite{Ellner_2016} and SI section \ref{SI-SIsect:scaling_factors} for the two alternative definitions of $q_{ij}$ that we use. 

The storage effect %for species $i$ 
is the contribution of covariation between $E$ and 
$C$ to a difference between invader and resident species mean growth rates  \cite{Ellner_2016}. %, when $i$ is the invader.
Specifically, storage effect for species $i$ is 
\begin{equation}
...
%Though some readers may find it slightly confusing to simultaneously use the variable $E_i$ and the operator $\E(\cdot)$, both of these notational choices are standard, so it would be more confusing to use alternative notation. Like all variables, $E_i$ is rendered in italic; like all operators, $\E(\cdot)$ is rendered upright.

We now specify how to decompose storage effect into contributions due to ATAs and 
due to $EC$ correlation \emph{per se}. 
Storage effect for species $i$ was computed by comparing
differences between invader and resident species mean growth rates for the original model and for a 
modified model for which 
%the components of $(E_i,C_{i \bs i})$ and those of $(E_j,C_{j \bs i})$
$E$ and $C$
were rendered independent of each other,
for both invader and resident species [see (\ref{eq:storage_effects_gen})]. 
Because %these components 
$E$ and $C$ 
were rendered completely independent in the modified model, 
storage effect quantifies the contribution of the whole $EC$ relationship, in all its aspects, 
to the invader/resident growth rate difference. 
But we can instead consider distributions $(E_i^\nat,C_{i \bs i}^\nat)$ and 
$(E_j^\nat,C_{j \bs i}^\nat)$ with the same
...
$E$ and $C$
altered relative to the original distributions. 
Specifically, asymmetries of tail association are removed. 
We then define $\olrn_{i \bs i}$ and $\olrn_{j \bs i}$ based on a modified model 
using  $(E_i^\nat,C_{i \bs i}^\nat)$
...
\end{align}
The term $\Delta_i^{[EC]}$ is the component of storage effect due to 
ATAs between $E_i$ and $C_{i \bs i}$, and between $E_j$ and $C_{j \bs i}$.
The term $\Delta_i^{[E \nat C]}$ is the component of storage effect due to $EC$ 
...
of storage effect, which are themselves a term in the MCT decomposition \citep{Ellner_2019}.
...
$C_{i \bs i}$, which we denote $F_{E_i}$ and $F_{C_{i \bs i}}$, 
respectively, are strictly monotonic and continuous. 
...
These are standard Normal 
distributed, although they are not necessarily jointly bivariate normal.  
Let $\rho_i$ be the covariance of these random variables. 
We then define a bivariate normal random variable $(e_i,c_{i \bs i})$ such that
...
and $C_{i \bs i}^\nat=F_{C_{i \bs i}}^{-1} \circ \varphi(c_{i \bs i})$. 
The value of $\ddot{\rho}_i$ is then determined in one of four alternative ways: 
either by setting 
$\ddot{\rho}_i = \rho_i$; 
or so that the Pearson, Spearman or Kendall correlation of 
$E_i^\nat$ and $C_{i \bs i}^\nat$ matches that of $E_i$ and $C_{i \bs i}$. 
The choice of bivariate normal 
$(e_i,c_{i \bs i})$ ensures that $E_i^\nat$ and $C_{i \bs i}^\nat$ are symmetrically tail associated.


For those familiar with copula statistics 
(see, e.g., \citealp{Genest_2007} and \citealp{Ghosh_2020_AER}), 
...
make no reference to covariation between quantities, 
(see eqn. (13) in Ellner (2019),
whereas ATAs are %defined in a covariation context
strictly about covariation. 
Therefore, the storage effect is the only mechanism of coexistence
...
which equals GWR of species $i$. The decomposition, which we use below, is analogous to and motivated by the 
...
$\Delta_i^{(EC)}$ is a contribution to coexistence of $EC$ covariation itself -- the storage effect. 
Only storage effect involve $EC$ covariation,
so the only mechanism of coexistence influenced by ATA between $E$ and $C$ is the storage effect. Combining our decomposition 
...
\end{equation}
which is the extension of MCT which was goal G1 of the Introduction. 
%One typically %(but see below) must examine this quantity for each species, $i$,to obtain complete information about coexistence. 
To pursue goal G2, below, we will quantify
the magnitude of the new ATA term, $\Delta_i^{[EC]}$, relative to the magnitudes of the other terms. 

\section{Methods}\label{section:methods}

%G2
To achieve goal G2 from the Introduction, 
left- or right-tail associated %noise 
log fecundities $(b_1,b_2)$, with $b_i$ normally distributed with mean $\mu_i$ and standard deviation 
...
%to coexistence that come from ATAs versus other mechanisms, 
We then plotted the terms $\Delta_i^0$, $\Delta_i^E$, $\Delta_i^C$, 
$\Delta_i^{(E\shp C)}$, $\Delta_i^{[E \nat C]}$ and $\Delta_i^{[EC]}$ [see (\ref{eq:full_decomp})] for the lottery model
against the parameters of the model, $\mu_1$, $\mu_2$, $\sigma$ and $\delta$. %; and we compared values on plots. 
...
instead they depend only on $\mu_1-\mu_2$ (SI section \ref{SI-sect:math_simplifications}). 
So we plotted 
the terms
against $\mu_1 - \mu_2$. 
Third, we showed that the values of all components in (\ref{eq:full_decomp}) were the same 
for both our left- and right-tail associated cases (SI section \ref{SI-sect:math_simplifications}), 
so we only generated plots for left-tail associated noise. 
For each combination of $\delta=0.2, 0.4, 0.6$
and $\mu_1-\mu_2=0,-0.5,-1$,
...
competitors, storage effect (blue plus red
lines on Fig. \ref{fig:fig3}) was the only means by which coexistence could occur, and ATA effects
tended to be negative, weakening total storage effect.
...
we decomposed the storage effect into two new mechanisms, the contribution of ATAs to coexistence,
...
ATA contributions to the storage effect and to species coexistence and the maintenance of biodiversity
...
Though storage effect is typically described as the contribution
of $EC$ \emph{covariance} to coexistence \citep{Chesson_2012, Ellner_2016, Ellner_2019},
our results suggest that storage effect should be redefined to constitute the 
...
Storage effect as computed using the original, analytic approach to MCT 
...
\begin{figure}
\includegraphics[width=\textwidth]{../results_figs/theory_fig_col.pdf}
\caption{Pedagogical figure to demonstrate how asymmetric tail associations are removed while maintaining original marginal distributions and correlation between variables. (a) the starting variables with ATA. (b) the variables are transformed by a normal inverse cumulative distribution function of their own cumulative distribution function. (with a Spearman correlation of $\rho). Asymmetric association structure is maintained. (c) a newly generated, naturally symmetric, bivariate normal with the correlation, $\ddot{\rho}=\rho. (d) two variables which have the same marginals and correlation as the starting variables in (a) but have a symmetric association structure, obtained by taking the inverse cumulative distribution functions of the starting variables of the normal cumulative distribution function of the normal distributions. For demonstration purposes we start with beta distributed variables, though variables can be distributed differently and the two can be dissimilar. \label{fig:theory}}
\end{figure}
...
Standard errors of plotted quantities were always less than \Sexpr{round(se5,5)}, so are not shown.
...
Standard errors of plotted quantities were always less than \Sexpr{round(se6,5)}, so are not shown.} \label{fig:fig6}
\end{figure}
