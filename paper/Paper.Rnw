...
We extend the MCT, decomposing storage
effects to show the influence of ATAs. Analysis of a simple model
...
tropical trees \citep{Usinowicz_2012}, plankton \citep{Descamps_2005, Caceres_1997, Narawani_2013}, and corals \citep{Alvarez-Noriega_2020}. 
...
and stability of ecosystem functioning \citep{Ghosh_2020_ecolevol,Ghosh_2021}.
...
%***DAN: Go though and make sure all citation callouts list papers in date order, olderst to newest,
...
left-tail association between these variables. 
As we will show in this study, changes in ATAs can then
...
[e.g., \cite{White_2001, Adler_2006, Jentsch_2007, Angert_2009, diPaola_2012, Parepa_2013, Descombes_2020, Usinowicz_2021}]. 
...
%***DAN: this para commented out for length, added a couple of the sentences to an above para to substitute
...
%***DAN: consider sending the lottery model components of this section to sup mat, to reduce length and complexity.
%Before doing that, make sure you understand what the consequences will be.
...
competitive pressure on species $i$, $C_i(t)$. We assume, for simplicity, 
...
%throughout the paper. Has to be done in the main text and sup mat and the figures themselves (IGR is often labeled on 
%the figure panels themselves).
