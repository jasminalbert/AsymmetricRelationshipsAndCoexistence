...
so we have appended a version of this main text rendered using latex to the end of the 
Supporting Information (SI); we recommend that readers interested in the mathematical content
of the study read that version.
%***DAN: Don't forget to do this

%Some intuition
We next give some initial intuition about how ATA effects on coexistence work, with a fuller understanding
necessarily coming only later in the paper.
It is known that storage effects can occur if some buffering mechanism reduces population 
...
when $E$ and $C$ are
both lower than average (the cases 2) are not necessarily equal.
Depending on the nature of population growth, storage potential may differ
across the spectrum from 1 to 2.
%
ATA effects are essentially the extent to which asymmetries of association between $E$ and 
...
storage potential is high, ATA effects will be low, and vice versa.
...
And we return in the Discussion to the task
of building intuition for how ATAs work. 
