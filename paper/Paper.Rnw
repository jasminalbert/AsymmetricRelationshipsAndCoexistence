...
G1) To augment MCT to show formally how ATAs plays a role in coexistence. 
...
plays a role in coexistence. For pedgogical clarity, we develop each step of our new theory for a general 
...
competitive pressure on species $i$, $C_i(t)$. As for the lottery model example (Introduction), we here assume, for simpicity, 
