...
%***Add to these
\keywords{competition, coexistence, biodiversity, modern theory of coexistence, storage effects, asymmetric tail association}
...
framework for understanding how species nevertheless coexist, and biodiversity in maintained, in real ecosystems. 
...
applied to multiple systems, including annual plants \citep{Angert_2009, Hallett_2019}, perennial plants \citep{Adler_2010, Adler_2006}, 
tropical trees \citep{Usinowicz_2012}, plankton \citep{Narawani_2013, Caceres_1997}, and corals \citep{Alvarez-Noriega_2020}. 
Although the MTC is mathematically complex to many ecologists, recent extensions make it more accessible \citep{Ellner_2016,Ellner_2019}.
...
of association are also used, in addition to correlations
...
and related ideas had previously been very seldom 
considered in ecology and related areas (but see \citealp{Anderson_2019, deValpine_2014, Popovic_2019}), 
...
whichever species is currently rare; this is the characteristic that make storage effects a so-called \emph{stabilizing} mechanism 
...
%This is reasonable assumption for non-rare species, since 
%a better environment leads to greater numbers which leads to more intraspecific competition, but a rare species may be essentially unaffected
...
in times when the environment is poor; dormant stages such as are in seed banks are an
example buffering mechanisms \citep{Chesson_2012}. 
...
Importantly, storage effects are, in fact, quantified by measuring to what extent EC covariance contributes to differences between 
invader and resident growth rates \citep{Chesson_1994,Ellner_2016}.
Thus ATAs may, in principle, influence storage effects simply because storage effects hinge on EC covariation, so ATAs 
between between E and C may mediate the strength of storage effects.

%***DAN: Jasmin to edit the below as follows:
%1) Topic sentence: Another potentially important reason to quatify the importance of ATA to species coexistence is that there may
%be reason to believe that climate change can alter ATAs of environmental variables implicated in coexistence, as follows. 
%2) Just adapt below text, maybe try to simplify a bit.
%3) This para could just focus on the mechanics of how ATAs change for two variables measured at different times. 
%***DAN: Keeping this sentence so Jasmin can put it somwhere, not here.
...
%***DAN: I am also having a hard time identifying the topic sentence of this paragraph, can you tell me which one it is, please,
%in a comment right under this one?
% Extreme events can have important ecological impacts for storage effect and coexistence by introducing ATAs. 
Furthermore, an additional driving reason to quantify the significance of ATAs for species coexistence is the potential for climate change to alter ATAs of environmental variables implicated in coexistence, as follows. 
The consequence of climate change that has the most potential to influence ATAs in the environment the increase in intensity, duration, extent, and frequency of extreme climatic events \citep{Jentsch_2007, Meehl_2004, Ummenhofer_2017}, i.e., heat-waves, floods, hurricanes, fires, etc. 
Temporally extensive extreme events can have important ecological impacts for
competing populations where species growth and the maintenance of biodiversity is dependent of phenologically separated environmental signals. A temporally extensive event can equalize the phenologically separated environmental signals, making those phenological signals more correlated. For example, consider two precipitations signals, one in early spring and one in late summer. If a long flood occurs, it's likely that both signals will be close to equal in that year. If extreme floods occur in more years than not, then the two signal will become more correlated in higher precipitation values, a right-tailed ATA. Similarly, if extreme droughts occur in more years than not, then the signals will be more correlated in the lower values, a left-tailed ATA. The same can be said about spatially separated signals and more spatially extensive extreme events. ATA of environmental signals can then introduce or change ATAs in the fitness or traits of species that depend on the signals and potentially affect their coexistence.
%***DAN:
%1) Topic sentence: 
To make the above ideas more concrete, we review a relevant example from the literature that may be
influenced as previously described. 
We introduce a simplified example of an Arizona desert annual plant community from \cite{Chesson_2012}. 
%2) Make it concrete - name the system, name the organisms, name the seasons. 
The community follows the criteria required for temporal storage effects to operate. Different species dominate and have high germination rates depending on rainfall timing. For example, Baileya multiradiata dominates if sufficient rainfall is reached in the early germination season. Eriogonum abertianum dominates is sufficient rainfall is reached in the late germination season. Competition is influenced by resource consumption of seed-producing adult plants. Dormant seeds are unaffected by competiton and can persist in the community in an unfavorable environment. When rain fall dynamics temporally fluctuate from year to year, relative abundance and dominance of the desert annual community also fluctuate year to year. The result is a relationship between the desert annuals' environmental responses (germination) that is symmetrically associated. However, if environmental responses become asymmetrically associated by, say, extreme rainfall events, coexistence can be altered. If the community is disturbed by frequent and long floods with some years of normalcy, the desert annual response relationship may become more correlated in the higher values or right-tail associated. Conversely, if the community is disturbed by frequent and long drought with some years of normalcy, the desert annual environmental response relationship may become more correlated in the lower values or left tail associated. 

To make the above ideas more concrete, we review a relevant example from the literature that may be
influenced as previously described. 
We introduce an example of a two species grass-forb plant community (inspired by \cite{Chesson_2012}).
%2) Make it concrete - name the system, name the organisms, name the seasons. 
To contextualize idea of how ATA in environmental response might manifest and modfiy the outcome of coexistence, The community follows the criteria required for temporal storage effects to operate. The forb dominates and has high fecundity when a sufficient threhold of rain is reached in the early season. The grass dominates and has high fecundity when there is sufficeint rain in the late season. Competition is influenced by resource comsumption of growing juvenile plants. Dormant seeds are unaffected by competiton and can persist in the community in an unfavorable environment. When rain fall dynamics temporally fluctuate from year to year, relative abundance and dominance of the grass-forb community also fluctuate year to year. The result is a relationship between the grass's and forb's environmental responses (fecundity) that is symmetrically associated, and, following our simulated example (Figure \ref{fig:pedagog}), the species persist in coexistence. However, if environmental responses become asymmetrically associated by, say, extreme rainfall events, coexistence can be altered. If the community is disturbed by frequent and long floods with some years of normalcy, the grass-forb environmental response relationship may become more correlated in the higher values or right-tail associated. Conversely, if the community is disturbed by frequent and long drought with some years of normalcy, the grass-forb environmental response relationship may become more correlated in the lower values or left tail associated. Again, we see the possible consequences of ATA in Figure \ref{fig:pedagog}, i.e., less or no coexistence. 

are more temporally extensive than moderate disturbances, then it is more likely that two species will be influenced by the same events. Both species may become more alike (correlated), an important consideration for competition and coexistence. For example, an extreme flood event that lasts across the growth season of two different species and increases recruitment in both species will cause recruitment of the two species to become more correlated in the upper tail end of the distribution. A subsequent change in competitive ability and coexistence is not just attributed to the extreme event alone, but from the resulting extreme relationship between species (ATA). 
%***DAN: Leave the following for the next para
Though previous literature considers impacts of extreme events, change in means, and increased variability on species coexistence and competition and community dynamics in general (see above), they do not directly consider ATA between species response caused by environmental changes -- extreme relationships caused by extreme events (but see \cite{Ghosh_2021} for impact of ATA on community aggregate properties).



%***JAS: I am trying to relay the message that climate change and specifically extreme events have consequences for coexistence and competition
% Maybe a better way to start this paragraph is:
% EC covariance, and thus the storage effect, may be altered by changes in the environment. 
% need an indication that we are more interested in increased variance and extreme events because we are looking we are looking for changes in a co-varying relationship??
%***DAN: Possible topic sentence like thing after talking to Jasmin on 9/17: 
%Although, to our knowledge, the potential importance of climatic changes in ATAs on coexistence has not been studied,
%the importance for coexistence of other aspects of climate change, including changes in means and variability of environmental variables, 
%have been studied in several systems. 
%***DAN: Do it like you have it but shorter.
%the RAMPAGE of extreme events such as heat-waves, floods, hurricanes, fires, etc. implications for coexistence and competition. 
Although, to our knowledge, the potential importance of climatic changes in ATAs on coexistence has not been studied, the importance for coexistence of other aspects of climate change, including changes in means and variability of environmental variables, have been studied in several systems.  
Environmental fluctuations are directly impacted by climate change through changes in means and increased variance \citep{Hansen_2012}. While both changes can be attributed to extreme events, shifts in mean are more-so a result of gradual trend effects, and increased variance is a result of extreme event effects \citep{Jentsch_2007} .
Impacts on competition and coexistence of climate change have been more commonly researched via studying changes in the mean of environmental variables; impacts of increased variance is less studied, although the rate of study has increased. \citep{Jentsch_2007}  %importance has been realized and variability is becoming more commonly studied now


%Jasmin tried to split this para up into effects on coexistence of changes in mean, starting here...
For example, decreased mean growth season precipitation can decrease the competitive ability of dominant forest tree species \citep{Clark_2011}, and as such, \cite{diPaola_2012} predict the collapse of coexistence between Mediterranean tree species as mean rainfall levels are forecasted to decrease. 
On the contrary, dominant species exclusion may be beneficial as rare species can increase in diversity\citep{Mariotte_2013}. Changes in environmental means can also increase fitness of a dominant species and in turn reduce community diversity, typical of invasive species \citep{Vitousek_1997}. 
As a result of changing means, range shifts can favor plant coexistence by introducing new herbivores\cite{Descombes_2020} or disfavor coexistence through increased competition \cite{Usinowicz_2021}.  
%...and variance...
In contrast, increased environmental variance has been less commonly studied; however, more research on the effect of increased variance has been urged and received recent focus through the study of extreme climatic events \citep{Jentsch_2007, Smith_2011}. Extreme drought and heat waves impact competitive output and community composition of grassland plants\citep{White_2000, White_2001, Hoover_2014}. However, complete exclusion of dominant species due to extreme events is uncommon \citep{Lloret_2012}. \cite{Fay_2003} experimentally manipulated rainfall for more frequent and intense precipitation while keeping constant the total volume of rain over Kansas tallgrass prairie and found that while some species had reduced fitness, overall community structure remained the same. Moreover, in regard to invasive species, increased climate and resource variability allowed for greater success of invasive plants in terms of density and competitive equilibrium \citep{Parepa_2013}. 
%...and extreme events (though this blends together with changes in variance)
Extreme events constitute large disturbances, quantitatively amounting to large fluctuations in environmental drivers and their ecological responses \citep{Smith_2011}. Although, increased variability of environmental fluctuations is a newer consideration for studying environmental changes; environmental variability has been analytically shown to be important for classical coexistence \citep{Chesson_1981, Warner_1985}. Moreover, increased variability has been found to promote coexistence in plants communities \citep{Adler_2006,  Angert_2009}, but disrupt coexistence in birds \citep{Stenseth_2002, Stenseth_2015}. This present study has applications to extreme events and climate change as we consider relationships in their extremes, potentially caused by extreme events, and their implications for competition and coexistence.
%DAN: So what jasmin was trying to do was summarize a lot of lit on effects of climate change on coexistence, in these three parts
%(means, varinace, and extreme events). 

%Move up the lottery model para from Theory to here, suture up the rift that this will produce in theory
To further demonstrate plausibility of ATA altering coexistence between species, we carried out simulations using a simple two-species model, the lottery model. 
Environmental variability has been classically shown to promote coexistence via storage mechanisms in a temporal lottery model of competition \citep{Chesson_1981, Warner_1985}, 
hence we move forward with this simple model to examine the effect of extreme relationships between environmental responses on the contribution of storage effects to coexistence. 
We introduce the lottery model and describe simulations based on it that clearly show that ATAs can influence 
species coexistence, though the details of this influence are the subject of the rest of the paper.
...
%very succinct description of sims you do (mostly it's in the figure caption)
%1-2 sentences on the differences observed
We simulated two different communities up to \Sexpr{M} time steps to hopefully make it clear to the reader that ATA makes a difference for competition and coexistence between species (see Figure \ref{fig:pedagog}). The only distinction between the two simulations is that one depends on asymmetrically tail associated temporal dynamics and one does not. Markedly, the simulation with ATA has longer periods of one-species dominance than the simulation without, implying less recoverability when rare and thus less coexistence \citep{Chesson_2000} when environmentally dependent variables are asymmetrically tail associated.     


%Goal para
We have demonstrated that ATAs exist and are common in nature \citep{Ghosh_2020_AER}, and we have elucidated how ATAs between environmental response variables might arise via extreme events. ATA in environmental response directly feeds into an ATA effect on the EC covariance contribution to the storage effect mechanism of coexistence. Thus we seek to explore the consequences of ATAs on coexistence, competition, and the maintenance of biodiversity.
...
G4) To provide an empirical example of species coexistence, and to highlight the role ATAs play in coexistence for the example.
In the Discussion, we revisit the potential that climatic changes will influence
the roles of ATAs in species coexistence and the maintenance of biodiversity.
Overall, our study presents a new mechanism of coexistence and biodiversity maintenance which may be influenced by climate change, 
and a means for understanding its importance for real systems. This is not a good ending sentence. 

\section{Theory}\label{section:theory}

We here pursue goal G1 from the Introduction, to augment the MTC to show how ATA
plays a role. For pedgogical clarity, we develop each step of the theory in general and for 
a simple example model, the lottery model (see \ref{section:introduction}). 
...
%***DAN: Say this here if it turns out to be true, or say something like it:
...
\ref{fig:fig3} shows that the ATA term of (\ref{eq:full_decomp}) sometimes, but not always, has 
magnitude comparable to the magnitudes of other terms of the extended MTC decomposition. 
Magnitudes of $\Delta_i^{[E \nat C]}$ tended to be larger, relative to the magnitudes of the 
...
lines on Fig. \ref{fig:fig3}) were the only means by which coexisitence could occur, and ATA effects
...
more common for larger $\delta$. Although bands of ATA rescue or exlusion on Fig.
...
%Need something like the following, maybe
...
\caption{Simple example of bivariate noise with standard normal marginals. A) is left-tail 
associated, B) is symmetric, and C) is right-tail associated noise. All three cases have 
the same Pearson correlation, up to sampling variation, yet show different very different 
patterns of association. We used asymmetrically tail associated cases which are perfectly 
correlated in the lower (A)  or upper (C) halfs of the distributions, though it is also 
possible to generate asymmetrically tail associated noise with imperfect correlations in 
both halves of the distributions. The variables $(b_1,b_2)$ were obtianed by generating 
noise as pictured and then multipling by $\sigma$ and adding $(\mu_1,\mu_2)$ (SI 
section \ref{SI-sect:noise}).\label{fig:noise}}
\end{figure}
...
\label{fig:pedagog}}
\end{figure}
%***DAN: Dan has not yet edited this caption, will do so when it is time to edit the Intro 
...
other components of the MTC decomposition. All panels show the IGR and the decomposition of
the IGR provided by the extended MTC, equation (\ref{eq:full_decomp}). For each panel, 
$\delta$ and $\mu_1-\mu_2$ 
