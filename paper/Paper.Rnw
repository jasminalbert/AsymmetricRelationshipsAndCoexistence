...
%***JASMIN: Done in text. Need to finish change in figures.
...
%***DAN: I see many instances of IGR have been replaced by GWR, but there are some IGRs remaining, e.g., 
%on the figures.
%***JASMIN: Figure 3 and 4 changed
...
We produced and applied a new extension to MCT that 
...
important information on the structure of relationships between variables, and our results add
...
%***DAN: Dan made it to here in editing and has to stop until after spring break
...
%Because asymmetric tail associations exist in many ecological variables \citep{Ghosh_2020_AER}, the more specified approach of considering unique drivers of coexistence rather than E and C and the ability to consider more than two drivers, could produce interesting results?? 
...
The chances of ATAs being important for the coexistence of species increases as more variables are included in the system, 
either through the addition of more species or more drivers (beyond just E and C), or both. 
When multiple species are included in the system, the interaction between different species' E's and C's introduces more relationships 
to consider that may or may not be asymmetrically related and could increase the relative importance of ATA contribution to the species' coexistence. 
Likewise, coexistence between species can also depend on more than two drivers \citep{Ellner_2019}. 
The storage effect decomposition presented here is generalizable to coexistence decompositions of multiple variables.
Following the set up and notation of \cite{Ellner_2019}'s \emph{covariance decomposition}, 
ATA contributions and correlation \emph{per se} can be decomposed from the \emph{associations} between two variables
among many from the appropriate term with parenthesized superscripts with the # symbol denoting and separating 
variables that are varying independently from the variables that are covarying whose relationship we are interested in decomposing. 
For example, the term $\Delta^{(XY#Z)}$ could be decomposed into $\Delta^{[XY#Z]} + \Delta^{[X \nat Y#Z]}$. 
As more terms and species are introduced into a system, more relationships between variables must be evaluated,
more of those relationships may be asymmetrically tail associated, and ATAs may contribute more to the maintenance of 
biodiversity in that system. 
...
As an introductory investigation of ATAs within the framework of MCT, 
we used simple and extreme examples, and future work should assess more complex relationships, systems, and models. 
...
or integral projection models should be explored )and varying degrees and qualities of ATA, (copula parameters, 
difference between tails, rate of relatedness(slope of expansion??) should be implemented and explored.
Moreover, because our empirical application makes use of a simple and controlled lab system, future studies
should be done to assess the extent of how much ATA are important for complex systems and field systems.
Because modeling the dynamics of long-lived populations can be challenging, a good field system to start
exploring the ideas presented here is a system of annual plants. 
For example, the ATAs between rainfall timing on the coexistence of grasses and forbs 
in California grasslands \citep{Hallett_2019} could be a fruitful 
further investigation of biodiversity maintenance in California grasslands and the effects of extreme 
rainfall fluctuation given the highly variable climate of the area.
Moreover, our approach may provide more insight when expanded to consider fluctuations across space,
in regards to questions about population synchrony, Moran effects, and the suggestion of possible ATA specialists. 
