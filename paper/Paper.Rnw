...
\noindent \textbf{Words in Abstract:} 150; \textbf{Words in main text:} 4975; \textbf{Words in text boxes:} 0\\
...
very commonly use metrics such as correlation. But standard measures of the strength 
...
Metrics of association other than correlations are also often used (e.g., various forms of 
regression), but other common methods also reveal only a small portion 
of the information content of the relationship \citep{Nelsen2006}.
ATAs and related ideas had previously been seldom considered in ecology, but recent 
...
Ghosh_2020_ecolevol, Ghosh_2021}. For instance, \cite{Ghosh_2020_Eco} argued that extinction risks can become elevated 
...
And \cite{Ghosh_2021} showed that ATAs can alter the stability of 
...
use data of a different type from what we focus on in this paper.
...
Competition and coexistence are research topics at the core of modern ecology 
...
instead drive the other extinct \citep{Hardin_1960}.
...
benefits from favorable time periods in order to persist through unfavorable periods. 
...
and on the assumption that no two species respond in the same way to environmental variation. Then, good environmental
...
water, when water is scarce, but their niches become more differentiated when water is abundant.
...
simple simulations using a two-species model, the lottery model. The simulations described in this
...
so we appended a latex version of this main text to the end of the 
Supporting Information (SI); readers interested in the math should read that version.
%***DAN: Don't forget to do this

%Some intuition
We next give some initial intuition about how ATA effects on coexistence work, 
with more details in the Discussion.
...
but this can occur 1) when $E$ is favorable and $C$ is not too strong,
...
when $E$ and $C$ are
both lower than average (the cases 2) are not necessarily equal -- storage potential may differ
...
varies across the $EC$ spectrum spanned by the cases 1-2 sketched above.
...
We here pursue goal G1 from the Introduction: to augment MCT to show the role of ATAs for coexistence. 
...
Because components were rendered 
...
The symbol should correspondingly be pronounced ``partial sharp''.
...
The question of uniqueness of the distribution $(E_i^\nat,C_{i \bs i}^\nat)$ 
...
any other symmetrically tail associated bivariate 
...
The derivation of this overall decomposition is in SI section 
...
scenario (see Methods). But we also defined scenarios of ``ATA rescue''
...
corresponded to more variable fecundities for species $i$. 
