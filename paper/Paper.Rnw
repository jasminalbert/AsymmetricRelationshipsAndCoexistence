...
%***Old text below, kept for now, but should be pretty much replaced by the above
...
%In a simple two species system, differences in environmental response allow for one species to grow to high densities in its favorable environemnt while the other is very low. %**JAS: should we introduce invader analysis or terminology here?
...
%Conversely, EC covariance is lesser for the low density species, leading to large fluctuations in population 
%growth rate in both directions. 
...
%For example, insensitive adults or dormants seeds can still persist when in an unfavorable environment and overall growth rate is negative \citep{Chesson_2012}; environment and competiton do not effect the buffered life stages.   
...
%***DAN: I think the above paragraphs are pretty solid. I am still working on the below.
...
%***DAN: Jasmin, there are a lot of disjointed sentences in the below paragraph, at least they appear disjointed to me.
%Can you please say in a comment right below this one what the topic sentence is? As usual, if you are having trouble 
%determining what the topic sentence it, it may mean you don't have one, and you need to determine what you're doing 
%with this paragraph and make a topic sentence. 
