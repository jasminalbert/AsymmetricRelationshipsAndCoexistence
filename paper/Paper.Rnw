...
Although the MTC is mathematically complex to most ecologists, recent extensions make it more accessible \citep{Ellner_2016,Ellner_2019}.
...
whichever species is currently rare; this is the characteristic that separates so-called \emph{stabilizing} mechanisms of coexistence from
\emph{equalizing} mechanisms, which instead reduce average fitness differences between comeptitors \citep{Chesson_2000}.
Storage effects can occur if three conditions are met \citep{Chesson_1994}. First, there must be positive covariation between the quality of the environment (E)
...
in times when the environment is poor; environmentally insensitive adult stages and dormant stages such as seed banks are
example mechanisms \citep{Chesson_2012}. 
When present, these factors combine so that EC covariance provides a relative benefit to whatever species is currently rare.
Storage effects are quantified by measuring to what extent EC covariance contributes to differences between 
growth rates of species when they are at low ``invader'' densities, and growth rates of the abundant ``resident'' species \citep{Chesson_1994,Ellner_2016}. 
Thus ATAs may influence storage effects because storage effects involve EC covariation. ATAs between E and C may,
in principle, mediate the strength of storage effects, though to our knowledge this possibility has not previously been investigated.
%***
