...
$\theta(t)=\theta_0 (t) +  a\sin(2\pi t/P)$, 
...
coexistence and exclusion of the weaker competitor. 
...
\caption{The degree of ATA contribution to the IGR change with parameters of environmental variation, i.e., temperature oscillation. The ratio $\Delta_i^{[EC]}/\text{IGR}$ constitutes the degree of ATA contribution to thee IGR with larger magnitudes signifying larger ATA contributions. In this model, for the plotted parameters, positive $\Delta_i^{[EC]}/\text{IGR}$ only result from positive $\Delta_i^{[EC]}$ and positive IGR and negative $\Delta_i^{[EC]}/\text{IGR}$ only result from positive $\Delta_i^{[EC]}$ and negative IGR. Thus, cyan areas indicate failure to coexist and magenta areas indicate coexistence with ATA contribution increasing with color intensity. The most intense areas of magenta indicate parameter subspaces where ATA rescue occurs. For simplicity and readability magnitudes of $\Delta_i^{[EC]}/\text{IGR}$ >1 were capped at $|1|$. } \label{fig:fig6}
\end{figure}
