...
case of panel H, species 1 is excluded with ATA but the species can coexist without them. 
...
coexistence and exclusion of the weaker competitor. Standard error was never greater than
\Sexpr{round(se3,5)}, a small value compared to y axis extents and comparisons between lines,
so standard errors were not displayed. \label{fig:fig3}}
\end{figure}
