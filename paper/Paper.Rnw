...
The coupling of ATA contribution magnitude with environmental variability, 
...
ecological phenomena as environmental conditions may evidently
become more variable (probably need refs).
As described in section \ref{section:introduction}, ATAs between environmental
variables may shift as the climate becomes more variable and extreme events become more 
expansive and extensive, causing associations of distinct temporal events to become 
more related in one extreme or tail, thus asymmetric. Additionally, 
...
possible [bivariate] \emph{relationships} in the extremes; however, 
...
extreme and changing climate is probable to affect other mechanisms in other ways (refs?).
...
environment, via a population's vital rates. Environment and environmental response may be related linearly
...
Threshold relationships can produce tail association, as we showed elsewhere \citep{Walter_2022}. 
...
vital rates should also be investigated for species coexistence and the maintenance of biodiversity.
...
on any additions to coexistence beyond simple correlations and does not include our new ATA term, $\Delta^{[EC]}$, or variance \emph{per se}, $\Delta^{(E \shp C)}$ \citep{Chesson_1994, Chesson_2000}. 
...
obscuring the effects of ATAs. Because standard MCT's assumption make it impossible to 
detect interactions outide of correlation \emph{per se}, simulation-based MCT is required to
detect ATA contributions, and our new extension to it clarifies and cleary separates and quantifies the 
relative importance of both ATAs and pure correlation in contribution to the storage effect
...
among many from the appropriate term with parenthesized superscripts containing the $\shp$ symbol which denotes and separates 
...
should be done to assess the extent of how much ATAs are important for complex systems and field systems which are 
often subject to factors beyond our control.
...
For example, the ATAs effects of rainfall timing on the coexistence of grasses and forbs 
