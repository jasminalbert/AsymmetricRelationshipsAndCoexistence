...
\underline{summary - include 1-2 sentences about misnomer}
...
simpler changes in extreme events themselves such as
...
environment (vital rates). Environment and environmental response may be related linearly
(like lottery model?); though probably real environmental variables are 
actually related to vital rates in a non-linear and often threshold-like way. 
Threshold relationships can produce tail association, as we showed elsewhere. 
...
Classical analytic derivations of MCT using Taylor expansion approximations completely miss out
on any additions to coexistence beyond simple correlations. 
...
\underline{conclude: how ur work answers/informs the broad question from the intro and lay out 2-3 key priorities for next research on this question}
