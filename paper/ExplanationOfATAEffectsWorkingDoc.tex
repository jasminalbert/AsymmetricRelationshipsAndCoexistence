...
\subsection{Dan thoughts 2023 07 26}
...
\subsection{SI section X corresponding to the thoughts above}
...
$E_i$ and $C_{j \bs i}$ should be monotonically related. So 
$\E[r_j(E_j^\nat,C_{j \bs i}^\nat)] = \E[r_j(E_j,C_{j \bs i})]=0$, giving
$\Delta_i^{[EC]}=0$. So temporal
niche differentiation is necessary for ATA effects on coexistence to 
materialize.
...
environmental fluctuation; but how do ATA effects then happen? We here extend the intuition
introduced in the Discussion and add detail. 
...
which features prominently in $\E[r_i(E_i,C_{i \bs i})]=\log(1-\delta+E_i/C_{i \bs i})$,
