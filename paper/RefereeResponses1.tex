...
is certainly idea. We have gone to great lengths to do so, and think we have succeeded. Though the referee's comment above may 
...
second-to-last paragraph in the Introduction) that begins establishing such an intuition. We also added a new paragraph to the 
Discussion (the second paragraph of the Discussion). These paragraphs are also supported by two new SI sections and two new SI figures which 
...
\noindent ***\emph{This is certainly a valid question, and we have gone to some lengths to address it. 
...
for the log-normal fecundities lottery model. We believe additional explanations and intuitive 
explorations could and should be the subject of several future papers, but we've added material to this manuscript
that takes that process as far as possible within the scope of the current paper, given length limits and
requests from the referees to simplify.} \\
%DONE
