...
such an acknowledgment is both rare from a referee and very useful for causing authors to most usefully revise their 
...
by resources and predators can more readily occur. }
...
Storage effects are ATA effects plus what we call the effects of $EC$ correlation \emph{per se} 
($\Delta_i^{[E \nat C]} in our notation), and either of these two summands can be negative or positive.
So if the twe effects are oppostite in sign and nearly equal, storage effects will be unimportant but ATA
effects will be important for coexistence.}
...
coexistence and to show that ATAs \text{may} be broadly important for coexistence, by showing examples where they
...
goals, so we thank the referee for not doing that.}

\emph{The new Discussion material is in a new SI section which is cited from the main text Discussion section. 
...
%DAN: Write the discussion material which is promised and the citation from the Discussion.
...
%DAN: maybe add to the above to point out that they don't need to know this to understand what we're doing in this paper!
...
%Are we being too petulant, should we just do it and say we did it?
...
%Jasmin to consider (don't do yet) how hard/error prone this change would be, taking into account the 
%formulas in the SI? Unless this is MUCH easier and MUCH less error prone than Dan's hunch, I suggest 
%we don't do it.   
...
%Jasmin to consider this while brainstorming the prior comment.
