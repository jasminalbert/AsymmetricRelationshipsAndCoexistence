...
computed a linear interpolation to define temperature dependence 
functions of $Q_i(t)$, $V_i(t)$, and $K_i(t)$. These values and functions are 
...
$C_j(t)=(K_j(t)+S(t))/S(t)$ and $r_j(E_j,C_j)=\frac{V_j(E)}{C_j} -D$. 
...
but should real variables that influence population growth.
Thus in our exploration, environmental fluctuation is temperature fluctuation.
...
when combine to make ordered pairs, form the desired sample from 
$(E_i^\nat,C_{i \bs i}^\nat)$. Because $(E_{i,1}^\nat,\ldots,E_{i,T}^\nat)$
