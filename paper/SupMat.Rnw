...
are those plotted using open points in Fig. \ref{MT-fig:pedagog1}d-e.
...
now over all indices $i$ for which $u_i+v_i>1$. These are the filled points 
in Fig. \ref{MT-fig:pedagog1}d-e. The left and right partial Spearman
...
Right-tail associated noise, $\left(b_1^{(i)}, b_2^{(i)}\right)$, $i=1,..,M$ (Fig. \ref{MT-fig:pedagog1}c) 
was generated by taking the negative of left-tail noise.
...
Fig. \ref{fig:lognormal_fecundidities_explanatory}. A careful reader may notice the
upper horizontal-axis histograms on Fig. \ref{fig:lognormal_fecundidities_explanatory}D, F
are mirror images of each other. This was because the right-tail noise with standard normal
marginals that we used was generated
by simply taking the negative of our left-tail noise with standard-normal marginals (SI section 
\ref{sect:noise}), a choice that could not realistically have influenced results in a meaningful way.
This can also be seen by comparing main text Fig. \ref{MT-fig:pedagog1}a, c.
