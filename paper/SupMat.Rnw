...
\title{Asymmetric relationships and their impact on coexistence: Supporting information}
\date{}
...
%***DAN: I decided to present the general theory (not for the lottery model but for the
%general case) in the Theory section in the main text, putting off the specific lottery
%model application to later. For that, I need to explain scaling factors in general.
%So this section is now for that explanation. That means the below text, which is about
%the scaling factors for the lottery model need to be handled later. It is good, however,
%that the below text is for the general lottery model, for either way of doing the 
%fecundities. That will come in handy later.
%
...
%***DAN: stopped here on 2023 01 27. See outline in the main text of how I need to restructure the 
%SI sections from this point forward.
