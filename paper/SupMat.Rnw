...
%***DAN: Jasmin, please start on this section when you get a chance. Here are some topics that need
%to be covered:
Estimations of $Q_i(t)$, $V_i(t)$, and $K_i(t)$, temperature dependence functions for cell quota, reproduction rate, and half saturation, respectively, were used to paramaterize the competing diatom model outline in the paper. 
Values of and subsequent function estimations are plotted in Figure \ref{fig:VQK}. 
Descamps and Julien obtained estimated values for $Q_i$, $V_i$, and $K_i$ from single species nine-day batch experiments at four different temperatures (6, 12, 18, 24) (see Table 1 of Descamps and Julien) and further computed a linear interpolation to define temperature dependence functions of $Q_i(t)$, $V_i(t)$, and $K_i(t)$. These values and functions are indicated in Figure \ref{fig:VKQ} by open circles and dashed lines. Ellner 2016 and 2019 chose to slightly modify some values and functions, indicated in Figure \ref{fig:VKQ} by closed circles and solid lines. 

%1) Precise delineation of the functions Q_i, V_i, K_i which were used. There are different options considered
%all over the place in the code and by Ellner et al (2019) and by the original diatom study, so we have to
%be very clear here. To give one example of the need for care, here, Ellner et al (2019) state on p. 6
%of their paper that K_1 and K_2 were constant for the range of temperature considered, and they state
%that they took constant values for these things. However, we used a larger range of temperatures. 
%Depending on which of K1fun/K2fun versus K1flatfun/K2flatfun (from DDataAndVKQfuns.R) was actually used
%may make a difference, so we have to specify. Note that even K1flatfun/K2flatfun were not actually flat
%for the whole temperature range 6-24 degrees C, though they were for the range of temperature Ellner
%et al considered. We should not say we used flat K1 and K2 functions unless we are sure we only ever
%used temperatures in the flat ranges of these functions. Better just to describe the functions we
%actually used, where the best way to describe them *may* be by plotting them. Anyway, what you need to
%do is make sure you know what versions of each of these 6 functions your code actually used, and then
%describe the functions precisely in this section.
...
the removal of asymmetric tail associations from the bivariate random variable
...
when the distributions of $E_i$ and $C_{i \bs i})$ are explicitly known, but for the 
diatom example explored for goal G4 of the main text (see also SI section \ref{sect:diatom_model}), 
we only have samples from $(E_i,C_{i \bs i})$, based on simulations. We here describe how to
produce a sample from $(E_i^\nat,C_{i \bs i}^\nat)$ in this modified context. 



In the paper section \ref{MT-section:theory}, we describe analytical methods for generating the partial sharp variables, $E_i^\nat$ and $C_{i \bs i}^\nat$, which are distributed in the same way as the defined $E_i$ and $C_{i \bs i}$, but are symmetric in the tails and have a normal copula structure.
The protocol for samples of coexistence drivers in empirical applications is as follows.
Let $\olE_i$ be a sample of a environmental fluctuation and $\olC_{i \bs i}$ be a sample of competitive pressure. Let $\Theta_{\olE}$ and $\Theta_{\olC}$ be standard normal distributions obtained by normalizing the ranks of $\olE_i$ and $\olC_{i \bs i}$, 
such that $\Theta_{\olE} = \frac{rank(\olE_i)}{n_{\olE_i}+1} \circ \varphi^{-1}$ and $\Theta_{\olC} = \frac{rank(\olC_{i \bs i})}{n_{\olC_{i \bs i}}+1} \circ \varphi^{-1}$, where $\varphi$ is the CDF of a standard normal distribution. The normalized correlation of $\olE_i$ and $\olC_{i \bs i}$, denoted by $r$, is the covariance of $\Theta_{\olE}$ and $\Theta_{\olC}$.  
We then simulate an $S$ number of bivariate normal distributions $N_s\left(\vec{0}, \Sigma \right)$
with covariance matrix $\Sigma = \begin{pmatrix} 1&\ddot{\rho} \\ \ddot{\rho}&1\end{pmatrix}$. 
The columns of $(\olE_{i}, \olC_{i \bs i})$ are permutated and saved $S$ amount of times, 
such that the rank of each column matches with the rank of each column of $N_s$. 
To obtain $(\olE_{i}^\nat, \olC_{i \bs i}^\nat)$, the median of the permuations is computed. 
Here, $\ddot\rho$ can be chosen a number of ways such that the Pearson, Spearman, or Kendall 
correlation of $(\olE_{i}^\nat, \olC_{i \bs i}^\nat)$ matches that of $(\olE_{i}, \olC_{i \bs i})$
or such that $\ddot\rho = r$. The code for this protocol is in \url{"diatom/partialSharp_fxns.R"} and was used in our application to the diatom chemostat system.
