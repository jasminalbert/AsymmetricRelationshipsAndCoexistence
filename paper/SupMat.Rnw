...
paramaterize the competing diatom model outlined in the paper. 
...
\cite{Ellne_2016, Ellner_2019} used slightly modified values and functions,
indicated in \ref{fig:VQK} by filled circles and solid lines. 
Our investigation follows that of \cite{Ellne_2016, Ellner_2019} and 
uses the modified functions of  $Q_i(t)$, $V_i(t)$, and $K_i(t)$ to analyze coexistence.
...
\caption[Species growth rate, silica uptake, and half sturation responses to changes in temperature.]
{Species maximum growth rate (a), half saturation constant (b), and silica uptake (c) responses to changes in temperature. Species 1 is \emph{F. crotonensis}, shown in blue, and species 2 is \emph{C. pseudostelligera}, shown in red. Open circles and dashed lines indicate estimated data and interpolations from \cite{Gonzalez_2005} and closed circles and solid lines indicate values and function used in our analysis as defined in \cite{Ellner_2016, Ellner_2019}. \label{fig:VKQ}}
\end{figure}
