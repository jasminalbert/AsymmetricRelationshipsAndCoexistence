...
\section{Decomposing storage effect in empirical applications}\label{sect:Empircal_app}

The decomposition of storage effects and quantification of contributions of ATAs depends on 
the removal of asymmetric tail associations from the relationship between
coexistence drivers while maintaining their correlation and marginal distributions. 
