...
%***DAN: Jasmin, panels d-l of this figure are difficult to read because the y axis extent is such that
%you can't really see what any of the lines are doing except for the ditted and dot-dashed lines. I think we
%need to present 2 versions of the figure, this one and another one. For the other one, the y-axis limits 
%should be determined from the y axis extents of all the lines other than the dotted and dot-dashed lines. 
%Can you please mak this alternative version and let me know when it is made and what the pdf file is, so I 
%insert it? It is possible you will also need two versions of the figure below, but we won't know that 
%until you make that figure.
\begin{figure}
...
%***DAN: The code does not yet produce this figure. Eventually it needs to.
%NOTE: I have assumed this figure will look substantially the same as the corresponding main text 
%figure when I wrote the Results. If it does not, then some revisions will be needed.
\begin{figure}
...
that figure for details. Standard errors of plotted quantities 
were never greater than \Sexpr{round(0,5)}, so are not displayed.}\label{fig:fig4_alt}
%***DAN: jasmin, once the se is generated and saved by your code, the chunk at the outset of this 
%document needs to be modified to load that quantity, and then the 0 in the above expression 
%needs to be replaced by the variable name used.
\end{figure}
