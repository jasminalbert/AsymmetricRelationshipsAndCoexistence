...
the removal of asymmetric tail associations from the bivariate distribution
$(E_i,C_{i \bs i})$ while maintaining its correlation and marginal distributions.
In the Theory section of the main text, we decribed methods for this removal process
when the distributions of $E_i$ and $C_{i \bs i})$ are explicitly known, but for the 
diatom example explored for goal G4 of the main text (see also SI section \ref{}), 
we only have samples of $E_i$ and $C_{i \bs i})$

;
a modified distribution $E_i^\nat$ and $C_{i \bs i}^\nat$
